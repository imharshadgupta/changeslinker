...
<div class="fldblks"><input type="text" name="txtInitiateDate" id="txtInitiateDate" class="fild" style="width:302px;" value="<?php echo set_value('txtInitiateDate',$InitiateDate); ?>" readonly="readonly" tabindex="1" /></div>
	...
	<?php
		...
		if($sql)
		{
			if(($sql->num_rows) > 0)
			{
				...
				$Client_Name = trim($row['cClientName']);
			?>
				<option value="<?php echo $Client_Id; ?>" <?php echo set_select("cmbClientProp","$Client_Id",($ClientPropId=="$Client_Id" ? TRUE:'')); ?>><?php echo $Client_Name; ?></option>
			<?php
				endforeach;	
			}
		}
...
<div class="fldblks">
<select name="cmbPossessionDone" id="cmbPossessionDone" class="fild" tabindex="13">
...
<div class="fldblks">
<select name="cmbPaymentReceivedCompletely" id="cmbPaymentReceivedCompletely" class="fild" tabindex="15">
...
function FormValidate(btn)
{
    ...
    else
    {
		...
		var url = "<?php echo base_url(); ?>/index.php/dashboard/edit_initiate_deal";
		...
		$.post(url,data,function(data){	
		    ...
		    if(data)
			{
				...
				{					
					...
					btn.value = 'Submit';
					alert(data.msg);
					var redirecturl = "<?php echo base_url(); ?>/index.php/dashboard/listing_initiate_deal";
