...
<fieldset><legend>Task Summary</legend>
			...
			<div class="row">
				...
				<div style="width:12%; height:100%; float:left; border:0px solid blue;">
					...
					<?php 
						...
						WHERE ( bActive=1 AND bDelete=0 
						
						)
						or iTaskId = $TaskId
						ORDER BY cTaskName ASC");
						...
						if($sql)
						{
							if(($sql->num_rows) > 0)
							{
								...
								$ctrl_Id = trim($row['iTaskId']);
								$ctrl_Name = trim($row['cTaskName']);
							?>
								<option value="<?php echo $ctrl_Id; ?>" <?php echo set_select("cmbTask","$ctrl_Id",($TaskId=="$ctrl_Id" ? TRUE:'')); ?>><?php echo $ctrl_Name; ?></option>
							<?php
								endforeach;	
							}
						}
				...
				<div style="width:13%; height:100%; float:left; border:0px solid blue;">
					...
					<?php 
					  //$sql = $this->db->query("SELECT requirement_master.iClientId,client_master.cClientName FROM requirement_master LEFT JOIN client_master ON requirement_master.iClientId=client_master.iClientId WHERE requirement_master.iClientId<>0 AND requirement_master.bActive=1 AND requirement_master.bDelete=0 GROUP BY requirement_master.iClientId ORDER BY client_master.cClientName");
						...
						$sql = $this->db->query("SELECT client_master.iClientId,client_master.cClientName 
						FROM client_master 
						
						WHERE ( client_master.bActive=1 AND client_master.bDelete=0 
						
						)
						or iClientId = $ClientReqId
						ORDER BY client_master.cClientName ASC");
				...
				<div style="width:13%; height:100%; float:left; border:0px solid blue;">
					...
					<?php 
						...
						WHERE ( bActive=1 
						AND bDelete=0 
						and iClientId = $ClientReqId )
						or iRequirementId = $RequirementId
						ORDER BY cRequirementTitle ASC");
						...
						if($sql)
						{
							if(($sql->num_rows) > 0)
							{
								...
								$ctrl_Id = trim($row['iRequirementId']);
								$ctrl_Name = trim($row['cRequirementTitle']);
							?>
								<option value="<?php echo $ctrl_Id; ?>" <?php echo set_select("cmbRequirement","$ctrl_Id",($RequirementId=="$ctrl_Id" ? TRUE:'')); ?>><?php echo $ctrl_Name; ?></option>
							<?php
								endforeach;	
							}
						}
				...
				<div style="width:13%; height:100%; float:left; border:0px solid blue;">
					...
					<?php 
						...
						AND bDelete=0 
						and iClientId = $ClientReqId )
						or iPropertyId = $PropertyId
						ORDER BY cPropertyName ASC");
						...
						if($sql)
						{
							if(($sql->num_rows) > 0)
							{
								...
								$ctrl_Id = trim($row['iPropertyId']);
								$ctrl_Name = trim($row['cPropertyName']);
							?>
								<option value="<?php echo $ctrl_Id; ?>" <?php echo set_select("cmbProperty","$ctrl_Id",($PropertyId=="$ctrl_Id" ? TRUE:'')); ?>><?php echo $ctrl_Name; ?></option>
							<?php
								endforeach;	
							}
						}
				...
				<option value="">Select Status</option>
					<?php 
						...
						$sql = $this->db->query("SELECT *
						FROM current_status_master
						WHERE ( bActive=1 
						AND bDelete=0 
						)
						or iCurrentStatusId = $CurrentStatusId
						ORDER BY cCurrentStatusName ASC");
						...
						if($sql)
						{
							if(($sql->num_rows) > 0)
							{
								...
								$ctrl_Id = trim($row['iCurrentStatusId']);
								$ctrl_Name = trim($row['cCurrentStatusName']);
							?>
								<option value="<?php echo $ctrl_Id; ?>" <?php echo set_select("cmbCurrentStatus","$ctrl_Id",($CurrentStatusId =="$ctrl_Id" ? TRUE:'')); ?>><?php echo $ctrl_Name; ?></option>
							<?php
								endforeach;	
							}
						}
