...
class Dashboard extends CI_Controller {
	...
	function index()
	{
		...
		$data['page_url']="client_master_listing";			
  ...
  /*function listing_party_type_master()
	...
	*/
	...
	{
		$ClientId = trim($this->input->post('cmbClient'));
		$data['result'] = $this->dashboard_model->get_requirement_by_client($ClientId);
	    $getdata=json_encode($data['result']);
	    echo $getdata;
	}
	...
	{
		$ClientId = trim($this->input->post('cmbClient'));
		$data['result'] = $this->dashboard_model->get_property_by_client($ClientId);
	    $getdata=json_encode($data['result']);
	    echo $getdata;
	}
...
//			redirect('dashboard/userpendingtasklist');
	...
	function listing_requirement_master($client_id = 0)
	{
		...
		$data['page_url']="requirement_master_listing";			
		$this->load->view('includes/template', $data);
	}
...
//                }
   ...
   function smtpmailer($to, $from, $from_name, $subject, $body) {
			...
			$mail->SetFrom('', 'Linkers');     //Who is sending the email
			$mail->AddReplyTo("', 'Linkers");  //Email address that receives the response
   ...
   function listing_property_master_datatable($client_id = 0)
	{
        ...
        function listing_property_master_datatable1($client_id = 0)
	{
       
       
    //   echo '3333333333333333333';die;
//		$data = array();
//		$data['message']="";
//		$data['query'] = $this->dashboard_model->get_all_property_master_datatable($client_id);
//		$data['page_url']="property_master_listing_datatable";			
//		$this->load->view('includes/template', $data);
                ...
                $this->dashboard_model->get_all_property_master_datatable1($aColumns, $sIndexColumn, $sTable,$sWhere,$_GET);
	}
