...
class Dashboard extends CI_Controller {
    ...
    function search() {
        if (isset($_POST['cmbFilter']) && ($_POST['cmbFilter'] != '')) {
            $Filter = trim($_REQUEST['cmbFilter']);

            $txtSearch = trim($_REQUEST['txtSearch']);

            $cmbPropertyCategory = trim($_REQUEST['cmbPropertyCategory']);
            $cmbPropertyType = trim($_REQUEST['cmbPropertyType']);
            $cmbPropertyStatus = trim($_REQUEST['cmbPropertyStatus']);
            $cmbAvailabilityStatus = trim($_REQUEST['cmbAvailabilityStatus']);
            $cmbState = trim($_REQUEST['cmbState']);
            $cmbCity = trim($_REQUEST['cmbCity']);
            $cmbLocation = trim($_REQUEST['cmbLocation']);

            if(isset($cmbCity) && !empty($cmbCity)){
                $data['cmbCity'] = $this->dashboard_model->get_city_by_id($cmbCity);    
            }
            if(isset($cmbLocation) && !empty($cmbLocation)){
                $data['cmbLocation'] = $this->dashboard_model->get_location_by_id($cmbLocation);
            }
            ...
            if ($Filter == 'Property') {
                ...
                $data['query'] = $this->dashboard_model->search_by_text_in_property($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbAvailabilityStatus, $cmbState, $cmbCity, $cmbLocation);
                $data['page_url'] = "search_listing";
            ...
            if ($Filter == 'Client') {
                ...
                $data['Filter'] = "Client";
                $data['txtSearch'] = $txtSearch;
                $data['query'] = $this->dashboard_model->search_by_text_in_client($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbAvailabilityStatus, $cmbState, $cmbCity, $cmbLocation);
                $data['page_url'] = "search_listing";
                $this->load->view('includes/template', $data);
            }
            ...
            if ($Filter == 'Requisition') {
                ...
                $data['txtSearch'] = $txtSearch;
                $data['query'] = $this->dashboard_model->search_by_text_in_requirement($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbAvailabilityStatus, $cmbState, $cmbCity, $cmbLocation);
                $data['page_url'] = "search_listing";
                $this->load->view('includes/template', $data);
            }
        } else {
    ...
    function get_all_requirement_ajax() {
        $data['result'] = $this->dashboard_model->get_all_requirement();
        ob_clean();
        $getdata = json_encode($data['result']);
        echo $getdata;
    }

    function get_all_property_ajax() {
        $data['result'] = $this->dashboard_model->get_all_property();
        ob_clean();
        $getdata = json_encode($data['result']);
        echo $getdata;
    }
    ...
    function getclientfrompsrmasterdata() {
        $data['clientid'] = $id = trim($this->input->post('clientid'));

        $data['result'] = $this->dashboard_model->get_clients_from_property_masterdata($id);
        ob_clean();
        $getdata = json_encode($data['result']);
        echo $getdata;
    }
    ...
    function searchrfpsr() {
        ...
        if (isset($_POST['cmbSearchFor']) && ($_POST['cmbSearchFor'] != '') && isset($_POST['cmbRFPSR']) && ($_POST['cmbRFPSR'] != '')) {
            ...
            if ($SearchFor == 'RF') {
                ...
                $data['RFPSRId'] = $RFPSRId = $_POST['cmbRFPSR'];
    ...
    function delete_existing_deal_done_attachment() {
        $AttachmentId = trim($this->input->post('AttachmentId'));
        $PropertyId = trim($this->input->post('PropertyId'));
        $AttachmentName = trim($this->input->post('AttachmentName'));
        $AttachmentPath = trim($this->input->post('AttachmentPath'));

        $result = $this->dashboard_model->dealdone_existing_attachment_delete($AttachmentId, $AttachmentPath);
    ...
    function delete_uploaded_dealdone_attachment() {
        $AttachmentName = trim($this->input->post('AttachmentName'));
        $AttachmentPath = trim($this->input->post('AttachmentPath'));
    ...
    function getcitybystateanddistrict() {
        ...
        $searchcheckcity = trim($this->input->post('searchcheckcity'));
        //echo $DistrictId;
        $data['result'] = $this->dashboard_model->get_city_by_state_and_district($StateId, $DistrictId, $searchcheckcity);
        ob_clean();
        $getdata = json_encode($data['result']);
        echo $getdata;
    }
    ...
    function getlocationbystateanddistrictandcity() {
        ...
        $searchcheckcity = trim($this->input->post('searchcheckcity'));
        //echo $DistrictId;
        $data['result'] = $this->dashboard_model->get_location_by_state_and_district_and_city($StateId, $DistrictId, $CityId, $searchcheckcity);
        ob_clean();
        $getdata = json_encode($data['result']);
        echo $getdata;
    }
    ...
    function listing_client_master_datatable() {
    	...
    	//if(extension_loaded('mbstring')) { echo 'loaded'; } else { echo 'not loaded'; }
        $data['query'] = $this->dashboard_model->get_all_client_master_datatable();
        //ob_clean();
        //$temp=str_replace("\ufeff", "", $data['query']);
        //preg_replace('/\u{feff}$/u', '', $data['query']);
        //$data['query']=json_decode($temp, TRUE);
        echo ($data['query']);
    }
    ...
    function edit_form_client_master($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['ClientCategory'] = trim($row['cClientCategory']);
                $data['Address'] = trim($row['cAddress']);
                ...
                for($i=1;$i<21;$i++)
                {
                    if(!empty($row['cContactPerson'.$i.'Name'])||!empty($row['cContactPerson'.$i.'Designation'])||!empty($row['cContactPerson'.$i.'PhoneNo1'])||!empty($row['cContactPerson'.$i.'PhoneNo2'])||!empty($row['cContactPerson'.$i.'Email'])){
                        $countFilledCP++;
                    }   
                }

                $data['countFilledCP'] = $countFilledCP;
                ...
                $data['ContactPerson11Name'] = trim($row['cContactPerson11Name']);
                $data['ContactPerson11Designation'] = trim($row['cContactPerson11Designation']);
                $data['ContactPerson11PhoneNo1'] = trim($row['cContactPerson11PhoneNo1']);
                $data['ContactPerson11PhoneNo2'] = trim($row['cContactPerson11PhoneNo2']);
                $data['ContactPerson11Email'] = trim($row['cContactPerson11Email']);

                $data['ContactPerson12Name'] = trim($row['cContactPerson12Name']);
                $data['ContactPerson12Designation'] = trim($row['cContactPerson12Designation']);
                $data['ContactPerson12PhoneNo1'] = trim($row['cContactPerson12PhoneNo1']);
                $data['ContactPerson12PhoneNo2'] = trim($row['cContactPerson12PhoneNo2']);
                $data['ContactPerson12Email'] = trim($row['cContactPerson12Email']);


                $data['ContactPerson13Name'] = trim($row['cContactPerson13Name']);
                $data['ContactPerson13Designation'] = trim($row['cContactPerson13Designation']);
                $data['ContactPerson13PhoneNo1'] = trim($row['cContactPerson13PhoneNo1']);
                $data['ContactPerson13PhoneNo2'] = trim($row['cContactPerson13PhoneNo2']);
                $data['ContactPerson13Email'] = trim($row['cContactPerson13Email']);


                $data['ContactPerson14Name'] = trim($row['cContactPerson14Name']);
                $data['ContactPerson14Designation'] = trim($row['cContactPerson14Designation']);
                $data['ContactPerson14PhoneNo1'] = trim($row['cContactPerson14PhoneNo1']);
                $data['ContactPerson14PhoneNo2'] = trim($row['cContactPerson14PhoneNo2']);
                $data['ContactPerson14Email'] = trim($row['cContactPerson14Email']);


                $data['ContactPerson15Name'] = trim($row['cContactPerson15Name']);
                $data['ContactPerson15Designation'] = trim($row['cContactPerson15Designation']);
                $data['ContactPerson15PhoneNo1'] = trim($row['cContactPerson15PhoneNo1']);
                $data['ContactPerson15PhoneNo2'] = trim($row['cContactPerson15PhoneNo2']);
                $data['ContactPerson15Email'] = trim($row['cContactPerson15Email']);


                $data['ContactPerson16Name'] = trim($row['cContactPerson16Name']);
                $data['ContactPerson16Designation'] = trim($row['cContactPerson16Designation']);
                $data['ContactPerson16PhoneNo1'] = trim($row['cContactPerson16PhoneNo1']);
                $data['ContactPerson16PhoneNo2'] = trim($row['cContactPerson16PhoneNo2']);
                $data['ContactPerson16Email'] = trim($row['cContactPerson16Email']);

                $data['ContactPerson17Name'] = trim($row['cContactPerson17Name']);
                $data['ContactPerson17Designation'] = trim($row['cContactPerson17Designation']);
                $data['ContactPerson17PhoneNo1'] = trim($row['cContactPerson17PhoneNo1']);
                $data['ContactPerson17PhoneNo2'] = trim($row['cContactPerson17PhoneNo2']);
                $data['ContactPerson17Email'] = trim($row['cContactPerson17Email']);


                $data['ContactPerson18Name'] = trim($row['cContactPerson18Name']);
                $data['ContactPerson18Designation'] = trim($row['cContactPerson18Designation']);
                $data['ContactPerson18PhoneNo1'] = trim($row['cContactPerson18PhoneNo1']);
                $data['ContactPerson18PhoneNo2'] = trim($row['cContactPerson18PhoneNo2']);
                $data['ContactPerson18Email'] = trim($row['cContactPerson18Email']);

                $data['ContactPerson19Name'] = trim($row['cContactPerson19Name']);
                $data['ContactPerson19Designation'] = trim($row['cContactPerson19Designation']);
                $data['ContactPerson19PhoneNo1'] = trim($row['cContactPerson19PhoneNo1']);
                $data['ContactPerson19PhoneNo2'] = trim($row['cContactPerson19PhoneNo2']);
                $data['ContactPerson19Email'] = trim($row['cContactPerson19Email']);


                $data['ContactPerson20Name'] = trim($row['cContactPerson20Name']);
                $data['ContactPerson20Designation'] = trim($row['cContactPerson20Designation']);
                $data['ContactPerson20PhoneNo1'] = trim($row['cContactPerson20PhoneNo1']);
                $data['ContactPerson20PhoneNo2'] = trim($row['cContactPerson20PhoneNo2']);
                $data['ContactPerson20Email'] = trim($row['cContactPerson20Email']);
                $data['BusinessPurposeId'] = trim($row['cBusinessPurpose']);
    ...
    function add_form_dcr_single($client_id, $ispopup = "") {
        ...
        $data['allrequirement'] = $this->dashboard_model->get_all_requirement();
        $data['allproperty'] = $this->dashboard_model->get_all_property();
    ...
    function add_task_assign() {
        if ($this->input->post()) {
            $data = $_POST;
            echo trim($this->dashboard_model->add_task_assign($data));
            exit();
        }
    }
    ...
    function edit_form_task_assign($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['dTaskDoneDate'] = trim($row['dTaskDoneDate']);
                $data['cDoneDetails'] = trim($row['cDoneDetails']);
...
//			redirect('dashboard/userpendingtasklist');
    ...
    function edit_form_property_master($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['AvailabilityStatus'] = trim($row['cAvailabilityStatus']);
                ...
                $data['NoOfFloorsInBuildings'] = trim($row['iNoOfFloorsInBuilding']);
    ...
    function viewproperty($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['TermsAndConditions'] = trim($row['cTermsAndConditions']);
                $data['Active'] = trim($row['bActive']);
                $data['cPropertyTaglineForWebsite'] = trim($row['cPropertyTaglineForWebsite']);
                $data['cSurroundingBrands'] = trim($row['cSurroundingBrands']);
    ...
    function listing_requirement_master_datatable($client_id = 0) {
        ...
        $data['query'] = $this->dashboard_model->get_all_requirement_master_datatable($client_id);
    }

    function requisitionhistory_inspections($reqId = 0){
    	$data = array();
        $data['message'] = "";
        $data['requirement_id'] = $reqId;
    	$data['result_data'] = $this->dashboard_model->requisitionhistory_inspections($reqId);
        $data['page_url'] = "requisitionhistory_inspections";
        $this->load->view('includes/template', $data);	
    }

    function requisitionhistory_deliveries($reqId = 0){
    	$data = array();
        $data['message'] = "";
        $data['requirement_id'] = $reqId;
    	$data['result_data'] = $this->dashboard_model->requisitionhistory_deliveries($reqId);
        $data['page_url'] = "requisitionhistory_deliveries";
        $this->load->view('includes/template', $data);	
    }
    ...
    function edit_form_requirement_master($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['CityId'] = trim($row['iCityId']);
                $data['LocationId'] = trim($row['iLocationId']);
                $data['Area'] = trim(mysql_real_escape_string($row['cArea']));
                $data['Height'] = trim($row['cHeight']);
                $data['Frontage'] = trim($row['cFrontage']);
    ...
    function viewrequisition($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['PowerLoad'] = trim($row['cPowerLoad']);
                $data['PowerBackup'] = trim($row['cPowerBackup']);
                $data['cRequirementTaglineForWebsites'] = trim($row['cRequirementTaglineForWebsite']);
    ...
    function edit_form_deal_initiate($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['allrequirement'] = $this->dashboard_model->get_all_requirement_deal();
                $data['allproperty'] = $this->dashboard_model->get_all_property_deal();
...
//  }
    ...
    function edit_form_deal_lost($id) {
        if ($id == null) {
            ...
            if ($row) {
                ...
                $data['Description'] = trim($row['cDescription']);
    ...
    function todaystatistics() {
        ...
        $report = $this->dashboard_model->dashboard_datewise_dcr_print_report($txtDate);
    ...
    function pendingrfreport_form() {
        $data = array();
        $data['message'] = "";
        $data['page_url'] = "pendingrfreport_form";
        $this->load->view('includes/template', $data);
    }
    ...
    function pendingrfreport() {
        ...
        if ($txtFromDate != "" && $txtToDate != "") {
            ...
            $report = $this->dashboard_model->pending_rf_print_report($txtFromDate, $txtToDate);
    ...
    function rfpendingacceptanceprintreport() {
        ...
        if ($txtFromDate != "" && $txtToDate != "") {
            ...
            $report = $this->dashboard_model->rf_pending_acceptance_print_report($txtFromDate, $txtToDate);
    ...
    function psrpendingacceptanceprintreport() {
        ...
        if ($txtFromDate != "" && $txtToDate != "") {
            ...
            $report = $this->dashboard_model->psr_pending_acceptance_print_report($txtFromDate, $txtToDate);
    ...
    function availablepropertiesreportform() {
        ...
        //$this->availablepropertiesprintreport();
        $this->load->view('includes/template', $data);
    }
    ...
    function availablepropertiesprintreport() {
        ...
        if ($txtFromDate != "" && $txtToDate != "") {
            ...
            $report = $this->dashboard_model->available_properties_print_report($txtFromDate, $txtToDate);
    ...
    function smtpmailer($RecipientEmailAddress, $RecipientEmailAddress, $EmailSubject, $EmailMessage, $body) {
    ...
    function listing_property_master_datatable($client_id = 0) {
        ...
        $data['query'] = $this->dashboard_model->get_all_property_master_datatable($client_id);
    }
...
// SQL server connection information
    ...
    function rftermcondition() {
        ...
        $report = $this->dashboard_model->rftermcondition();
    ...
    function psrtermcondition() {
        ...
        $report = $this->dashboard_model->psrtermcondition();
    ...
    function requirement_aging_report_form() {
        $data = array();
        $data['message'] = "";
        $data['page_url'] = "datewise_aging_report_rf_report_form";
        $this->load->view('includes/template', $data);
    }
    ...
    function requirement_aging_report() {
        ...
        if ($txtFromDate != "" && $txtToDate != "") {
            ...
            $report = $this->dashboard_model->requirement_aging_report($txtFromDate, $txtToDate);
    ...
    function property_aging_report_form() {
        $data = array();
        $data['message'] = "";
        $data['page_url'] = "datewise_aging_report_psr_report_form";
        $this->load->view('includes/template', $data);
    }

    function property_aging_report() {
        ob_start();
        ...
        if ($txtFromDate != "" && $txtToDate != "") {
            ...
            $report = $this->dashboard_model->property_aging_report($txtFromDate, $txtToDate);
    ...
    function property_available_report() {
        ...
        $report = $this->dashboard_model->property_available_report();
    ...
    function residential_property_report() {
        ...
        $report = $this->dashboard_model->residential_property_report();
    ...
    function need_sourcewise_reports_form() {
        $data = array();
        $data['message'] = "";
        $data['page_url'] = "need_sourcewise_reports_form";
        $this->load->view('includes/template', $data);
    }

    function need_sourcewise_reports() {
        ob_start();
        $data['cmbSource'] = $cmbSource = trim($this->input->post('cmbSource'));
        $data['txtDate'] = $txtDate = trim($this->input->post('txtDate'));
        $data['cmbFilter'] = $cmbFilter = trim($this->input->post('cmbFilter'));
        $data['txtToDate'] = $txtToDate = trim($this->input->post('txtToDate'));

        $report = $this->dashboard_model->need_sourcewise_reports($cmbSource, $txtDate, $cmbFilter, $txtToDate);
    ...
    function drilldown_report($client_id = 0) {

        $data = array();
        $data['message'] = "";
        $data['client_id'] = $client_id;

        $taskid = trim($this->input->get('taskid'));
        $userid = trim($this->input->get('user_id'));
        $formdate = trim($this->input->get('formdate'));
        $todate = trim($this->input->get('todate'));

        $data['taskid'] = trim($this->input->get('taskid'));
        $data['userid'] = trim($this->input->get('user_id'));
        $data['formdate'] = trim($this->input->get('formdate'));
        $data['todate'] = trim($this->input->get('todate'));

        //$data['cName'] = $this->dashboard_model->getUserName($userid);

        //$data['cTaskName'] = $this->dashboard_model->getTaskName($taskid);

        $data['page_url'] = "drilldown_listing_datatable";
        //ob_clean();
        $this->load->view('includes/template', $data);
    }

    function drilldown_report_datatable($taskid = 0, $userid = 0, $formdate = 0, $todate = 0) {
        ob_clean();
        $data['query'] = $this->dashboard_model->get_drill_down_datatable($taskid, $userid, $formdate, $todate, $aColumns, $sIndexColumn, $sTable, $sWhere, $_GET);
    }
    ...
    function dashboards() {
        ...
        $data['query'] = $this->dashboard_model->get_dashboardData();

        $data['page_url'] = "dashbaordhome";
        $this->load->view('includes/template', $data);
    }
    ...
    function testemail() {
       ...
       // echo phpinfo();
        $this->smtpmailer("rahul@rsspl.net", "rahul@rsspl.net", "tetsetstes", "tetsetstes", "testesse");
       // mail("rahul@rsspl.net","My subject","hello");
    }

    //------------------------------------------- Website Logos -------------------------------------------------------------------------------

    function listing_website_logo_master() {
        $data = array();
        $data['message'] = "";
        $data['query'] = $this->dashboard_model->get_all_website_logo_master();
        $data['page_url'] = "website_logo_master_listing";
        $this->load->view('includes/template', $data);
    }

    function add_form_website_logo_master() {
        $data = array();
        $data['message'] = "";
        $data['page_url'] = "website_logo_add_form";
        $this->load->view('includes/template', $data);
    }

    function add_website_logo_master() {
        if ($this->input->post()) {
            $data = $_POST;
            echo $this->dashboard_model->add_website_logo_master($data);
            exit;
        }
    }

    function edit_form_website_logo_master($id) {
        if ($id == null) {
            show_error('No identifier provided', 500);
        } else {
            $data = array();
            ...
            $row = $this->dashboard_model->website_logo_master_get_by_id($id);

            if ($row) {
                $data['WebsiteLogoId'] = trim($row['iWebsiteLogoId']);
                $data['WebsiteLogoFileName'] = trim($row['cWebsiteLogoFileName']);
                $data['WebsiteLogoFilePath'] = trim($row['cWebsiteLogoFilePath']);
                $data['Active'] = trim($row['bActive']);

                $data['page_url'] = 'website_logo_edit_form';
                $this->load->view('includes/template', $data);
            } else {
                show_error('Error in retrieving data by id');
            }
    ...
    function edit_website_logo_master() {
        if ($this->input->post()) {
            $data = $_POST;
            //echo print_r($data);
            echo $this->dashboard_model->edit_website_logo_master($data);
            exit;
        }
    }

    function delete_website_logo_master($id) {
        $delete = $this->dashboard_model->delete_website_logo_master($id);
        if ($delete) {
            redirect('dashboard/listing_website_logo_master');
        }
    }

    //////////////////////////////////////////////////////////////////////////////////////////////////

    public function upload_website_logo_file() {

        $status = "";
        $msg = "";
        $uploadedwebsitelogofilepath = "";
        $uploadedwebsitelogofilename = "";
        $file_element_name = 'txtWebsiteLogo';
      ...
      /*if(empty($_POST['title']))
        ...
        if (!isset($_FILES['txtWebsiteLogo']['name']) && empty($_FILES['txtWebsiteLogo']['name'])) {
            $status = "error";
            $msg = "Please select file";
        }

        if ($status != "error") {
            $config['file_name'] = 'website_logo'."_".time();
            $config['upload_path'] = './uploads/website_logos';
            $config['allowed_types'] = 'gif|jpg|png|';
            //$config['max_size'] = 1024 * 8;
            //$config['encrypt_name'] = TRUE;
            $config['max_size'] = '0';   // 0 = no limit on file size (this also depends on your PHP configuration)
            ...
            if (!$this->upload->do_upload($file_element_name)) {
                ...
                if ($data) {
                    $status = "success";
                    $msg = "Website Logo File successfully uploaded";
                    $uploadedwebsitelogofilepath = './uploads/website_logos/' . $data['file_name']; //$data['full_path']
                    $uploadedwebsitelogofilename = $data['file_name'];
                } else {
                    unlink($data['full_path']);
                    $status = "error";
                    $msg = "Something went wrong when saving the file, please try again.";
        ...
        echo json_encode(array('status' => $status, 'msg' => $msg, 'website_logo_file_path' => $uploadedwebsitelogofilepath, 'website_logo_file_name' => $uploadedwebsitelogofilename));
    }

    function delete_existing_website_logo() {
        $WebsiteLogoId = trim($this->input->post('WebsiteLogoId'));
        $WebsiteLogoName = trim($this->input->post('WebsiteLogoName'));
        $WebsiteLogoPath = trim($this->input->post('WebsiteLogoPath'));

        if(file_exists($WebsiteLogoPath)) {
            $delete = unlink($WebsiteLogoPath);

            if ($delete) {
                $sql = "DELETE FROM website_logo_master WHERE website_logo_master.iWebsiteLogoId='".$WebsiteLogoId."'";
    ...
    function delete_uploaded_website_logo() {
        $WebsiteLogoName = trim($this->input->post('WebsiteLogoName'));
        $WebsiteLogoPath = trim($this->input->post('WebsiteLogoPath'));
