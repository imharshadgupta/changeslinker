...
class Dashboard_model extends CI_Model {
    ...
    function sendEmailpreserver($RecipientEmailAddress, $EmailSubject, $EmailMessage) {
        $mail = new PHPMailer();
        $mail->IsSMTP();                                     // We are going to use SMTP
        $mail->SMTPAuth = true;                                 // Enabled SMTP authentication
        $mail->SMTPSecure = "";                                // Prefix for secure protocol to connect to the server
        $mail->Host = "linkersindia.in";                 // Setting GMail as our SMTP server
        $mail->Port = 25;     //25;            // SMTP port to connect to GMail
        $mail->Username = "support@linkersindia.in";           // User email address
        $mail->Password = "SupLin_123";
        $mail->IsHTML(true); // Password in GMail

        $mail->SetFrom('support@linkersindia.in', 'Linkers India');     //Who is sending the email
        $mail->AddReplyTo("support@linkersindia.in', 'Linkers India");  //Email address that receives the response
    ...
    function sendEmaildata($RecipientEmailAddress, $EmailSubject, $EmailMessage) {
        ...
        $mail->Port = 25;                                 // SMTP port to connect to GMail
        $mail->Username = "support@linkersindia.in";           // User email address
        $mail->Password = "SupLin_123";                          // Password in GMail
        $mail->IsHTML(true);
        $mail->SetFrom('support@linkersindia.in', 'Linkers India');     //Who is sending the email
    ...
    function get_city_by_state_and_district($StateId, $DistrictId, $searchcheckcity) {

        if ($searchcheckcity == 1) {
    ...
    function get_location_by_state_and_district_and_city($StateId, $DistrictId, $CityId, $searchcheckcity) {
    ...
    function get_contact_person_by_client($ClientId) {
        $sql = "SELECT client_master.cContactPerson1Name,client_master.cContactPerson2Name,client_master.cContactPerson3Name,client_master.cContactPerson4Name,client_master.cContactPerson5Name,
			  client_master.cContactPerson6Name,client_master.cContactPerson7Name,client_master.cContactPerson8Name,client_master.cContactPerson9Name,client_master.cContactPerson10Name,
                          client_master.cContactPerson11Name,
                          client_master.cContactPerson12Name,
                          client_master.cContactPerson13Name,
                          client_master.cContactPerson14Name,
                          client_master.cContactPerson15Name,
                          client_master.cContactPerson16Name,
                          client_master.cContactPerson17Name,
                          client_master.cContactPerson18Name,
                          client_master.cContactPerson19Name,
                          client_master.cContactPerson20Name        
                    FROM client_master WHERE client_master.iClientId='" . $ClientId . "'";
        ...
        if ($query->result()) {
            ...
            if (!empty($row['cContactPerson11Name']) && ($row['cContactPerson11Name'] != '')) {
                $CPArray[11] = $row['cContactPerson11Name'];
            }

            if (!empty($row['cContactPerson12Name']) && ($row['cContactPerson12Name'] != '')) {
                $CPArray[12] = $row['cContactPerson12Name'];
            }

            if (!empty($row['cContactPerson13Name']) && ($row['cContactPerson13Name'] != '')) {
                $CPArray[13] = $row['cContactPerson13Name'];
            }

            if (!empty($row['cContactPerson14Name']) && ($row['cContactPerson14Name'] != '')) {
                $CPArray[14] = $row['cContactPerson14Name'];
            }

            if (!empty($row['cContactPerson15Name']) && ($row['cContactPerson15Name'] != '')) {
                $CPArray[15] = $row['cContactPerson15Name'];
            }

            if (!empty($row['cContactPerson16Name']) && ($row['cContactPerson16Name'] != '')) {
                $CPArray[16] = $row['cContactPerson16Name'];
            }

            if (!empty($row['cContactPerson17Name']) && ($row['cContactPerson17Name'] != '')) {
                $CPArray[17] = $row['cContactPerson17Name'];
            }
            if (!empty($row['cContactPerson18Name']) && ($row['cContactPerson18Name'] != '')) {
                $CPArray[18] = $row['cContactPerson18Name'];
            }
            if (!empty($row['cContactPerson19Name']) && ($row['cContactPerson19Name'] != '')) {
                $CPArray[19] = $row['cContactPerson19Name'];
            }
            if (!empty($row['cContactPerson20Name']) && ($row['cContactPerson20Name'] != '')) {
                $CPArray[20] = $row['cContactPerson20Name'];
            }
    ...
    function search_by_text_in_property($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbAvailabilityStatus, $cmbState, $cmbCity, $cmbLocation) {
        $sql = "SELECT property_master.iPropertyId,property_master.cPropertyName,client_master.cClientName,
                state_master.cStateName,district_master.cDistrictName,city_master.cCityName,
                location_master.cLocationName,property_master.cPropertyAddress,source_master.cSourceName,
                property_category_master.cPropertyCategoryName,property_type_master.cPropertyTypeName,
                property_status_master.cPropertyStatusName,property_master.cPropertyPurpose,
                property_master.cPropertyLegalStatus,property_master.cPropertyRemarks,
                property_master.cTotalPlotArea,property_master.cBuildingArea,
                property_master.iNoOfFloorsInBuilding,property_master.cGroundCoverage,property_master.cFloorOffered,
                property_master.cPlateAreaOfFloorOffered,property_master.cToilet,property_master.cParking,
                property_master.cCarpetArea,property_master.cBuiltUpArea,property_master.cSuperBuiltUpArea,
                property_master.cFrontage,property_master.cDepth,property_master.cHeight,agreement_type_master.cAgreementTypeName,
                property_master.cDemandPerSqFeet,property_master.cDemandGross,property_master.cSecurityDeposit,
                escalation_master.cEscalationName,property_master.cCAM,property_master.cServiceTaxOnLessor,property_master.cServiceTaxOnLessee,property_master.cPropertyTaxOnLessor,property_master.cPropertyTaxOnLessee,property_master.cStampDutyAndRegistration,
                property_master.cLockIn,property_master.cLockInDuration,property_master.cRentFreePeriod,property_master.cNoticePeriod,property_master.cPossessionStatus,property_master.cPowerLoad,property_master.cPowerBackup,property_master.cCurrentTenant,property_master.dLeaseUpToDate,property_master.cPreviousTenant,property_master.dAgreementDate,property_master.cAgreementPlace,property_master.cPerson1DuringAgreement,property_master.cPerson2DuringAgreement,property_master.cAvailabilityStatus            
                ...
                WHERE 
                (
        (property_master.cPropertyName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (client_master.cClientName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (state_master.cStateName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (district_master.cDistrictName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (city_master.cCityName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (location_master.cLocationName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPropertyAddress LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (source_master.cSourceName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_category_master.cPropertyCategoryName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_type_master.cPropertyTypeName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_status_master.cPropertyStatusName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPropertyPurpose LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPropertyLegalStatus LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.cPropertyRemarks LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cTotalPlotArea LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cBuildingArea LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.iNoOfFloorsInBuilding LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cGroundCoverage LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cFloorOffered LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPlateAreaOfFloorOffered LIKE '%" . mysql_real_escape_string($txtSearch) . "%') 
        OR (property_master.cToilet LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cParking LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cCarpetArea LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cBuiltUpArea LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cSuperBuiltUpArea LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cFrontage LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cDepth LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cHeight LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (agreement_type_master.cAgreementTypeName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cHeight LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cDemandPerSqFeet LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cDemandGross LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cSecurityDeposit LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (escalation_master.cEscalationName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.cCAM LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cServiceTaxOnLessor LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cServiceTaxOnLessee LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPropertyTaxOnLessee LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.cStampDutyAndRegistration LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.cLockIn LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.cLockInDuration LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cRentFreePeriod LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cNoticePeriod LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPossessionStatus LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPowerLoad LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPowerBackup LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cCurrentTenant LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (property_master.dLeaseUpToDate LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPreviousTenant LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.dAgreementDate LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cAgreementPlace LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPerson1DuringAgreement LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (property_master.cPerson2DuringAgreement LIKE '%" . mysql_real_escape_string($txtSearch) . "%')
         )  ";


        $sql .= " AND ( ";

//        $sql .= "(property_master.iStateId = '" . $cmbState . "'
//        AND property_master.iCityId ='" . $cmbCity . "'
//        AND property_master.iLocationId = '" . $cmbLocation . "'
//        AND property_master.iPropertyCategoryId = '" . $cmbPropertyCategory . "'
//        AND property_master.iPropertyTypeId = '" . $cmbPropertyType . "'
//        AND property_master.iPropertyStatusId = '" . $cmbPropertyStatus . "'
//        AND property_master.bDelete=0)';


        $sql .= " property_master.bDelete=0 ";
        if ($cmbState != "") {
            $sql .= " AND property_master.iStateId = '" . $cmbState . "'";
        }
        if ($cmbCity != "") {
            $sql .= " AND property_master.iCityId ='" . $cmbCity . "'";
        }
        if ($cmbLocation != "") {
            $sql .= " AND property_master.iLocationId = '" . $cmbLocation . "'";
        }
        if ($cmbPropertyCategory != "") {
            $sql .= " AND property_master.iPropertyCategoryId = '" . $cmbPropertyCategory . "'";
        }
        if ($cmbPropertyType != "") {
            $sql .= " AND property_master.iPropertyTypeId = '" . $cmbPropertyType . "'";
        }
        if ($cmbPropertyStatus != "") {
            $sql .= " AND property_master.iPropertyStatusId = '" . $cmbPropertyStatus . "'";
        }
        if ($cmbAvailabilityStatus != "") {
            $sql .= " AND property_master.cAvailabilityStatus = '" . $cmbAvailabilityStatus . "'";
        }

        $sql .= " ) ";

        $sql .= " ORDER BY cPropertyName ASC";
    ...
    function search_by_text_in_client($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbAvailabilityStatus, $cmbState, $cmbCity, $cmbLocation) {

        $sql = "SELECT client_master.iClientId,client_master.cClientName,client_master.cAddress,
                state_master.cStateName,district_master.cDistrictName,city_master.cCityName,
                location_master.cLocationName,client_master.cLandmark,client_master.cContactPerson1Name,
                client_master.cContactPerson1Designation,client_master.cContactPerson1PhoneNo1,
                client_master.cContactPerson1PhoneNo2,client_master.cContactPerson1Email,
                client_master.cContactPerson2Name,client_master.cContactPerson2Designation,
                client_master.cContactPerson2PhoneNo1,client_master.cContactPerson2PhoneNo2,
                client_master.cContactPerson2Email,client_master.cContactPerson3Name,
                client_master.cContactPerson3Designation,client_master.cContactPerson3PhoneNo1,
                client_master.cContactPerson3PhoneNo2,client_master.cContactPerson3Email,
                client_master.cRemarks,property_master.cAvailabilityStatus
                FROM client_master LEFT JOIN state_master ON client_master.iStateId=state_master.iStateId 
                LEFT JOIN district_master ON client_master.iDistrictId=district_master.iDistrictId 
                LEFT JOIN city_master ON client_master.iCityId=city_master.iCityId 
                LEFT JOIN location_master ON client_master.iLocationId=location_master.iLocationId
                LEFT JOIN property_master ON property_master.iClientId=client_master.iClientId                
                WHERE 
                    (
                      (client_master.cClientName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cAddress LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (state_master.cStateName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (district_master.cDistrictName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (city_master.cCityName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (location_master.cLocationName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cLandmark LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson1Name LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson1Designation LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson1PhoneNo1 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson1PhoneNo2 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson1Email LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson2Name LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson2Designation LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson2PhoneNo1 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
                      (client_master.cContactPerson2PhoneNo2 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson2Email LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson3Name LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson3Designation LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                      (client_master.cContactPerson3PhoneNo1 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
                      (client_master.cContactPerson3PhoneNo2 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (client_master.cContactPerson3Email LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (client_master.cRemarks LIKE '%" . mysql_real_escape_string($txtSearch) . "%') 
                      ) ";
        ...
        $sql .= " ORDER BY client_master.cClientName ASC ";
    ...
    function search_by_text_in_requirement($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbAvailabilityStatus, $cmbState, $cmbCity, $cmbLocation) {
        ...
        $sql = "SELECT requirement_master.iRequirementId,requirement_master.dDate,client_master.cClientName,requirement_master.cContactPerson1,requirement_master.cContactPerson2,requirement_master.cContactPerson3,state_master.cStateName,district_master.cDistrictName,city_master.cCityName,location_master.cLocationName,requirement_master.cArea,requirement_master.cHeight,requirement_master.cFrontage,source_master.cSourceName,requirement_master.cPropertyPurpose,business_purpose_master.cBusinessPurposeName,property_category_master.cPropertyCategoryName,requirement_master.cRequirementType,requirement_master.cBudgetPerMonth,requirement_master.cFloorLevelPreference,escalation_master.cEscalationName,requirement_master.cLeasePeriodPreference,requirement_master.cRentFreeFitOutPeriod,requirement_master.cPowerLoad,requirement_master.cPowerBackup,requirement_master.dExpectedLaunchDate,requirement_master.cRemarks,requirement_master.cRegistrationExpensesToBeBorneBy,requirement_master.cTaxationToBeBorneBy,requirement_master.cLockInPeriod,requirement_master.cEstimatedInteriorBudget,requirement_master.cParkingPreference,requirement_master.dAgreementDate,requirement_master.cAgreementPlace,requirement_master.cPerson1DuringAgreement,requirement_master.cPerson2DuringAgreement,requirement_master.cTermsAndConditions,requirement_master.cServiceChargesForLinkers
				...
				WHERE ( 
                                    (requirement_master.dDate LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (client_master.cClientName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cContactPerson1 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cContactPerson2 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cContactPerson3 LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (state_master.cStateName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (district_master.cDistrictName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (city_master.cCityName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (location_master.cLocationName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cArea LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cHeight LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cFrontage LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (source_master.cSourceName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cPropertyPurpose LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (business_purpose_master.cBusinessPurposeName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                                        (property_category_master.cPropertyCategoryName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cRequirementType LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cBudgetPerMonth LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR (requirement_master.cFloorLevelPreference LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
    (escalation_master.cEscalationName LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (requirement_master.cLeasePeriodPreference LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
            (requirement_master.cRentFreeFitOutPeriod LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
                (requirement_master.cPowerLoad LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
(requirement_master.cPowerBackup LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
    (requirement_master.dExpectedLaunchDate LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (requirement_master.cRemarks LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
            (requirement_master.cRegistrationExpensesToBeBorneBy LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                (requirement_master.cTaxationToBeBorneBy LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                    (requirement_master.cLockInPeriod LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
    (requirement_master.cEstimatedInteriorBudget LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR
        (requirement_master.cParkingPreference LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
(requirement_master.dAgreementDate LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
    (requirement_master.cAgreementPlace LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
        (requirement_master.cPerson1DuringAgreement LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
            (requirement_master.cPerson2DuringAgreement LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                (requirement_master.cTermsAndConditions LIKE '%" . mysql_real_escape_string($txtSearch) . "%') OR 
                    (requirement_master.cServiceChargesForLinkers LIKE '%" . mysql_real_escape_string($txtSearch) . "%') 
                                    ) ";

        $sql .= " AND ( ";


        $sql .= " requirement_master.bDelete=0 ";

        if ($cmbState != "") {
            $sql .= " AND state_master.iStateId = '" . $cmbState . "'";
        }
        if ($cmbCity != "") {
            $sql .= " AND city_master.iCityId ='" . $cmbCity . "'";
        }
        if ($cmbLocation != "") {
            $sql .= " AND location_master.iLocationId = '" . $cmbLocation . "'";
        }
    ...
    function get_all_requirement_deal() {
    ...
    function get_all_property_deal() {
    ...
    function get_clients_from_property_masterdata($id) {
        $sql = "SELECT client_master.iClientId,client_master.cClientName FROM client_master WHERE client_master.iClientId = '" . $id . "' ORDER BY client_master.cClientName ASC";
    ...
    function get_all_client_master_datatable() {
        ...
        $aColumns = array("client_master.iClientId", "client_master.cClientName",
           ...
           "cContactPerson1PhoneNo1 AS ph1,cContactPerson1PhoneNo2 AS ph12,
			...
			cContactPerson9PhoneNo1 AS ph9,cContactPerson9PhoneNo2 AS ph92,
			cContactPerson10PhoneNo1 AS ph10,cContactPerson10PhoneNo2 AS ph102,
            cContactPerson11PhoneNo1 AS ph11,cContactPerson11PhoneNo2 AS ph112,
            cContactPerson12PhoneNo1 AS ph12,cContactPerson12PhoneNo2 AS ph122,
            cContactPerson13PhoneNo1 AS ph13,cContactPerson13PhoneNo2 AS ph132,
            cContactPerson14PhoneNo1 AS ph14,cContactPerson14PhoneNo2 AS ph142,
            cContactPerson15PhoneNo1 AS ph15,cContactPerson15PhoneNo2 AS ph152,
            cContactPerson16PhoneNo1 AS ph16,cContactPerson16PhoneNo2 AS ph162,
            cContactPerson17PhoneNo1 AS ph17,cContactPerson17PhoneNo2 AS ph172,
            cContactPerson18PhoneNo1 AS ph18,cContactPerson18PhoneNo2 AS ph182,
            cContactPerson19PhoneNo1 AS ph19,cContactPerson19PhoneNo2 AS ph192,
            cContactPerson20PhoneNo1 AS ph20,cContactPerson20PhoneNo2 AS ph202",
"( SELECT csm.cCurrentStatusName FROM dcr_summary ds  join dcr_detail dd on dd.iDCRId = ds.iDCRId
left outer join current_status_master csm on csm.iCurrentStatusId = dd.iCurrentStatusId
 where iClientReqId = client_master.iClientId
					and ds.bDelete=0 and dd.bDelete=0  
					and dd.iCurrentStatusId !=0
					order by dd.iDCRId desc limit 0,1 ) as totdcrrf",
            "( SELECT count(*) FROM dcr_summary ds  join dcr_detail dd on dd.iDCRId = ds.iDCRId
left outer join current_status_master csm on csm.iCurrentStatusId = dd.iCurrentStatusId
 where iClientReqId = client_master.iClientId
					and ds.bDelete=0 and dd.bDelete=0  
 ...
 where iClientReqId = client_master.iClientId
        ...
        if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {
            ...
            if (is_numeric($_GET['sSearch'])) {
                $sWhere .= " cContactPerson1PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson1PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR
                             cContactPerson2PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson2PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 OR
                                 cContactPerson3PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson3PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 OR
                                 cContactPerson4PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson4PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 OR
                                cContactPerson5PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson5PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                OR 
                                cContactPerson6PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson6PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson7PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson7PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson8PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson8PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson9PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson9PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson10PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson10PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                            OR 
                                cContactPerson11PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson11PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson12PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson12PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson13PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson13PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson14PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson14PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson15PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson15PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson16PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson16PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                             OR 
                                cContactPerson17PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson17PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                 
                            OR 
                                cContactPerson18PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson18PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                
                            OR 
                                cContactPerson19PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson19PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                                
                            OR 
                                cContactPerson20PhoneNo1 LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                             OR cContactPerson20PhoneNo2 LIKE '%" . $this->db->escape_like_str($sSearch) . "%'";
            } else {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            if (isset($_GET['bSearchable_' . $i]) && $_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '' && $i != 0 && $i != 1 && $i != 4) {
                if ($sWhere == "") {
                    $sWhere = $where;
                } else {
                    $sWhere .= " AND ";
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            if (trim($aRow['ph112'])) {
                $phone = $phone . "<br />," . trim($aRow['ph112']);
            }
            if (trim($aRow['ph122'])) {
                $phone = $phone . "<br />," . trim($aRow['ph122']);
            }
            if (trim($aRow['ph132'])) {
                $phone = $phone . "<br />," . trim($aRow['ph132']);
            }
            if (trim($aRow['ph142'])) {
                $phone = $phone . "<br />," . trim($aRow['ph142']);
            }
            if (trim($aRow['ph152'])) {
                $phone = $phone . "<br />," . trim($aRow['ph152']);
            }
            if (trim($aRow['ph162'])) {
                $phone = $phone . "<br />," . trim($aRow['ph162']);
            }
            if (trim($aRow['ph172'])) {
                $phone = $phone . "<br />," . trim($aRow['ph172']);
            }
            if (trim($aRow['ph182'])) {
                $phone = $phone . "<br />," . trim($aRow['ph182']);
            }
            if (trim($aRow['ph192'])) {
                $phone = $phone . "<br />," . trim($aRow['ph192']);
            }
            if (trim($aRow['ph202'])) {
                $phone = $phone . "<br />," . trim($aRow['ph202']);
            }
    ...
    function add_client_master($data) {
        ...
        $ClientCategory = trim($data['cmbClientCategory']);
        $Address = trim($data['txtAddress']);
        ...
        $ContactPerson1PhoneNo2 = trim($data['txtContactPerson1PhoneNo2']);
        $ContactPerson1Email = trim($data['txtContactPerson1Email']);
        $ContactPerson1Emails = trim($data['txtContactPerson1Email']);
        ...
        $ContactPerson11Name = trim($data['txtContactPerson11Name']);
        $ContactPerson11Designation = trim($data['txtContactPerson11Designation']);
        $ContactPerson11PhoneNo1 = trim($data['txtContactPerson11PhoneNo1']);
        $ContactPerson11PhoneNo2 = trim($data['txtContactPerson11PhoneNo2']);
        $ContactPerson11Email = trim($data['txtContactPerson11Email']);


        $ContactPerson12Name = trim($data['txtContactPerson12Name']);
        $ContactPerson12Designation = trim($data['txtContactPerson12Designation']);
        $ContactPerson12PhoneNo1 = trim($data['txtContactPerson12PhoneNo1']);
        $ContactPerson12PhoneNo2 = trim($data['txtContactPerson12PhoneNo2']);
        $ContactPerson12Email = trim($data['txtContactPerson12Email']);


        $ContactPerson13Name = trim($data['txtContactPerson13Name']);
        $ContactPerson13Designation = trim($data['txtContactPerson13Designation']);
        $ContactPerson13PhoneNo1 = trim($data['txtContactPerson13PhoneNo1']);
        $ContactPerson13PhoneNo2 = trim($data['txtContactPerson13PhoneNo2']);
        $ContactPerson13Email = trim($data['txtContactPerson13Email']);

        $ContactPerson14Name = trim($data['txtContactPerson14Name']);
        $ContactPerson14Designation = trim($data['txtContactPerson14Designation']);
        $ContactPerson14PhoneNo1 = trim($data['txtContactPerson14PhoneNo1']);
        $ContactPerson14PhoneNo2 = trim($data['txtContactPerson14PhoneNo2']);
        $ContactPerson14Email = trim($data['txtContactPerson14Email']);

        $ContactPerson15Name = trim($data['txtContactPerson15Name']);
        $ContactPerson15Designation = trim($data['txtContactPerson15Designation']);
        $ContactPerson15PhoneNo1 = trim($data['txtContactPerson15PhoneNo1']);
        $ContactPerson15PhoneNo2 = trim($data['txtContactPerson15PhoneNo2']);
        $ContactPerson15Email = trim($data['txtContactPerson15Email']);

        $ContactPerson16Name = trim($data['txtContactPerson16Name']);
        $ContactPerson16Designation = trim($data['txtContactPerson16Designation']);
        $ContactPerson16PhoneNo1 = trim($data['txtContactPerson16PhoneNo1']);
        $ContactPerson16PhoneNo2 = trim($data['txtContactPerson16PhoneNo2']);
        $ContactPerson16Email = trim($data['txtContactPerson16Email']);


        $ContactPerson17Name = trim($data['txtContactPerson17Name']);
        $ContactPerson17Designation = trim($data['txtContactPerson17Designation']);
        $ContactPerson17PhoneNo1 = trim($data['txtContactPerson17PhoneNo1']);
        $ContactPerson17PhoneNo2 = trim($data['txtContactPerson17PhoneNo2']);
        $ContactPerson17Email = trim($data['txtContactPerson17Email']);

        $ContactPerson18Name = trim($data['txtContactPerson18Name']);
        $ContactPerson18Designation = trim($data['txtContactPerson18Designation']);
        $ContactPerson18PhoneNo1 = trim($data['txtContactPerson18PhoneNo1']);
        $ContactPerson18PhoneNo2 = trim($data['txtContactPerson18PhoneNo2']);
        $ContactPerson18Email = trim($data['txtContactPerson18Email']);

        $ContactPerson19Name = trim($data['txtContactPerson19Name']);
        $ContactPerson19Designation = trim($data['txtContactPerson19Designation']);
        $ContactPerson19PhoneNo1 = trim($data['txtContactPerson19PhoneNo1']);
        $ContactPerson19PhoneNo2 = trim($data['txtContactPerson19PhoneNo2']);
        $ContactPerson19Email = trim($data['txtContactPerson19Email']);

        $ContactPerson20Name = trim($data['txtContactPerson20Name']);
        $ContactPerson20Designation = trim($data['txtContactPerson20Designation']);
        $ContactPerson20PhoneNo1 = trim($data['txtContactPerson20PhoneNo1']);
        $ContactPerson20PhoneNo2 = trim($data['txtContactPerson20PhoneNo2']);
        $ContactPerson20Email = trim($data['txtContactPerson20Email']);
        ...
        if ($query) {
            if ($query->num_rows > 0) {
                ...
                $addData = array(
                    ...
                    'cContactPerson11Name' => $ContactPerson11Name,
                    'cContactPerson11Designation' => $ContactPerson11Designation,
                    'cContactPerson11PhoneNo1' => $ContactPerson11PhoneNo1,
                    'cContactPerson11PhoneNo2' => $ContactPerson11PhoneNo2,
                    'cContactPerson11Email' => $ContactPerson11Email,
                    'cContactPerson12Name' => $ContactPerson12Name,
                    'cContactPerson12Designation' => $ContactPerson12Designation,
                    'cContactPerson12PhoneNo1' => $ContactPerson12PhoneNo1,
                    'cContactPerson12PhoneNo2' => $ContactPerson12PhoneNo2,
                    'cContactPerson12Email' => $ContactPerson12Email,
                    'cContactPerson13Name' => $ContactPerson13Name,
                    'cContactPerson13Designation' => $ContactPerson13Designation,
                    'cContactPerson13PhoneNo1' => $ContactPerson13PhoneNo1,
                    'cContactPerson13PhoneNo2' => $ContactPerson13PhoneNo2,
                    'cContactPerson13Email' => $ContactPerson13Email,
                    'cContactPerson14Name' => $ContactPerson14Name,
                    'cContactPerson14Designation' => $ContactPerson14Designation,
                    'cContactPerson14PhoneNo1' => $ContactPerson14PhoneNo1,
                    'cContactPerson14PhoneNo2' => $ContactPerson14PhoneNo2,
                    'cContactPerson14Email' => $ContactPerson14Email,
                    'cContactPerson15Name' => $ContactPerson15Name,
                    'cContactPerson15Designation' => $ContactPerson15Designation,
                    'cContactPerson15PhoneNo1' => $ContactPerson15PhoneNo1,
                    'cContactPerson15PhoneNo2' => $ContactPerson15PhoneNo2,
                    'cContactPerson15Email' => $ContactPerson15Email,
                    'cContactPerson16Name' => $ContactPerson16Name,
                    'cContactPerson16Designation' => $ContactPerson16Designation,
                    'cContactPerson16PhoneNo1' => $ContactPerson16PhoneNo1,
                    'cContactPerson16PhoneNo2' => $ContactPerson16PhoneNo2,
                    'cContactPerson16Email' => $ContactPerson16Email,
                    'cContactPerson17Name' => $ContactPerson17Name,
                    'cContactPerson17Designation' => $ContactPerson17Designation,
                    'cContactPerson17PhoneNo1' => $ContactPerson17PhoneNo1,
                    'cContactPerson17PhoneNo2' => $ContactPerson17PhoneNo2,
                    'cContactPerson17Email' => $ContactPerson17Email,
                    'cContactPerson18Name' => $ContactPerson18Name,
                    'cContactPerson18Designation' => $ContactPerson18Designation,
                    'cContactPerson18PhoneNo1' => $ContactPerson18PhoneNo1,
                    'cContactPerson18PhoneNo2' => $ContactPerson18PhoneNo2,
                    'cContactPerson18Email' => $ContactPerson18Email,
                    'cContactPerson19Name' => $ContactPerson19Name,
                    'cContactPerson19Designation' => $ContactPerson19Designation,
                    'cContactPerson19PhoneNo1' => $ContactPerson19PhoneNo1,
                    'cContactPerson19PhoneNo2' => $ContactPerson19PhoneNo2,
                    'cContactPerson19Email' => $ContactPerson19Email,
                    'cContactPerson20Name' => $ContactPerson20Name,
                    'cContactPerson20Designation' => $ContactPerson20Designation,
                    'cContactPerson20PhoneNo1' => $ContactPerson20PhoneNo1,
                    'cContactPerson20PhoneNo2' => $ContactPerson20PhoneNo2,
                    'cContactPerson20Email' => $ContactPerson20Email,
                    'cRemarks' => htmlentities($Remarks, ENT_QUOTES),
                    'cClientCategory' => $ClientCategory,
                    'bActive' => $Active,
                    'iUserId' => 0,
                    'cBusinessPurpose' => $BusinessPurposeId ? $BusinessPurposeId : 0
                );
                ...
                $mailhtml .= "<p>Greetings from Linkers India!</p>
                                <p>We are pleased to share that we have added you in our CRM as our prestigious Client.<br>We strive to deliver quality Corporate Leasing Services and assure you that our association shall be successful.</p>
                                <p>Thanks!</p>
                                <p>Best Regards,<br>";


                $mailhtml .= "LINKERS INDIA<br>211, Bakhatgarh Tower, 10/1, New Palasiya, Indore-452001<br>
            Ph. 0731-4044406, Cell: 8349998444, Email: <a href='mailto:support@linkersindia.in' target='_blank'>support@linkersindia.in</a>";

                $mailhtml .= '</p></div>';


                $this->sendEmail($ContactPerson1Emails, 'Welcome', $mailhtml);
                ...
                $innerstaff .= "<p>Best Regards,<br>";

                $innerstaff .= "LINKERS INDIA<br>211, Bakhatgarh Tower, 10/1, New Palasiya, Indore-452001<br>
            Ph. 0731-4044406, Cell: 8349998444, Email: <a href='mailto:support@linkersindia.in' target='_blank'>support@linkersindia.in</a>";
    ...
    function edit_client_master($data) {
        ...
        $DistrictId = trim($data['cmbDistrict']);
        $CityId = trim($data['cmbCity']);
        $LocationId = $data['cmbLocation'] ? trim($data['cmbLocation']) : 0;
        $Landmark = $data['txtLandmark'] ? trim($data['txtLandmark']) : "";
        $SourceId = $data['cmbSource'] ? trim($data['cmbSource']) : 0;
    ...
    /* function add_dcr($data)
      ...
      $mail->SMTPAuth   = true;                                 // Enabled SMTP authentication
      $mail->SMTPSecure = "ssl";                                // Prefix for secure protocol to connect to the server
      $mail->Host       = "mail.linkersindia.in";     		          // Setting GMail as our SMTP server
      $mail->Port       =  465;                                 // SMTP port to connect to GMail
      $mail->Username   = "support@linkersindia.in";           // User email address
      $mail->Password   = "sup123";                          // Password in GMail

      $mail->SetFrom('support@linkersindia.in', 'Linkers');     //Who is sending the email
      $mail->AddReplyTo("support@linkersindia.in', 'Linkers");  //Email address that receives the response
...
//        
//        echo '----------';
//        print_r($data['cmbProperty']);
//        echo "--------------------";
//        print_r($data['cmbRequirement']);
//        
//        die;

        if ($data['cmbProperty'][0] != "") {
            $propid = $data['cmbProperty'][0];
            $sql1 = "SELECT iClientId FROM property_master WHERE iPropertyId='" . $propid . "'";
            $query1 = $this->db->query($sql1);
            if ($query1->num_rows() > 0) {
                $row1 = $query1->row_array();
                $propclientid = trim($row1['iClientId']);
            }
        }
        if ($data['cmbRequirement'][0] != "") {
            $reqid = $data['cmbRequirement'][0];
            $sql1 = "SELECT iClientId FROM requirement_master WHERE iRequirementId='" . $reqid . "'";
            $query1 = $this->db->query($sql1);
            if ($query1->num_rows() > 0) {
                $row1 = $query1->row_array();
                $reqclientid = trim($row1['iClientId']);
            }
        }
        ...
        $cmbClientReq1 = $data['cmbClientReq'][0] ? $data['cmbClientReq'][0] : "";
            ...
            foreach ($data['cmbTask'] as $key => $val) {
                if (!empty($data['cmbTask'][$key]) && ($data['cmbTask'][$key] != '')) {
                    $addDCRDetail = array(
                        'iDCRId' => $DCRId,
                        'iTaskId' => $data['cmbTask'][$key],
                        'iClientReqId' => $reqclientid ? $reqclientid : $cmbClientReqs,
                        'iRequirementId' => $data['cmbRequirement'][$key],
                        'iClientPropId' => $propclientid ? $propclientid : '',
                        'iPropertyId' => $data['cmbProperty'][$key],
                        'iCurrentStatusId' => $data['cmbCurrentStatus'][$key],
                        'cDCRSummary' => $data['txtDCRSummary'][$key],
                        'clientid' => $cmbClientReq1,
                    );
...
//		}
        ...
        if ($Saveflag == true) {
            return trim(json_encode(Array("status" => "1", "msg" => SUCCESS_INSERT_MSG)));
        } else {
            return trim(json_encode(Array("status" => "0", "msg" => ERROR_UPDATE_MSG)));
        }
    }
    ...
    function get_all_task_assign() {
        $sql = "SELECT task_assign.iTaskAssignId,UMTAB.cName as cTaskAssignedByName,
                UMTDB.cName as cTaskDoneByName,
                UU.cName as cDepartmentName,CMREQ.cClientName as cReqClientName,
                requirement_master.cRequirementTitle,CMPROP.cClientName as cPropClientName,
                property_master.cPropertyName,task_assign.dTaskAssignDateTime,task_assign.cTaskSummary,
                task_assign.dTaskTargetDateTime,task_assign.dReminderDateTime,
                CASE task_assign.bTaskDone WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bTaskDone' 
                FROM task_assign 
                LEFT JOIN user_master as UMTAB ON task_assign.iTaskAssignedByUserId=UMTAB.iUserId 
                LEFT JOIN user_master as UMTDB ON task_assign.iTaskDoneByUserId=UMTDB.iUserId 
                LEFT JOIN user_master as UU ON task_assign.iDepartmentId=UU.iUserId 
                LEFT JOIN client_master as CMREQ ON task_assign.iClientReqId=CMREQ.iClientId 
                LEFT JOIN client_master as CMPROP ON task_assign.iClientPropId=CMPROP.iClientId 
                LEFT JOIN requirement_master ON task_assign.iRequirementId=requirement_master.iRequirementId 
                LEFT JOIN property_master ON task_assign.iPropertyId=property_master.iPropertyId 
                WHERE task_assign.bDelete=0 ORDER BY iTaskAssignId DESC";
    ...
    function add_task_assign($data) {
        ...
        $query = $this->db->query("SELECT cName FROM user_master WHERE user_master.iUserId='" . $DepartmentId . "'");
        $row = $query->row_array();
        $DepartmentName = trim($row['cName']);
        ...
        $taskdonestatus = $this->input->post('taskdonestatus');
        $donedate = $this->input->post('donedate');
        $donedetails = $this->input->post('donedetails');
        $donedates = trim($donedate);
        if (!empty($donedates)) {
            $txtReminderDtTms = str_replace('/', '-', $donedates);
            $ReminderDateTimes = date('Y-m-d H:i:s', strtotime($txtReminderDtTms));
        } else {
            $ReminderDateTimes = "";
        }
        ...
        $addData = array(
            ...
            'iRequirementId' => $RequirementId ? $RequirementId : 0,
            'iClientPropId' => $ClientPropId ? $ClientPropId : 0,
            'iPropertyId' => $PropertyId ? $PropertyId : 0,
            'iTaskId' => $TaskId ? $TaskId : 0,
            'cTaskSummary' => $TaskSummary,
            'dTaskTargetDateTime' => $TaskTargetDateTime ? $TaskTargetDateTime : 0,
            'dReminderDateTime' => $ReminderDateTime ? $ReminderDateTime : 0,
            'bTaskDone' => $taskdonestatus ? $taskdonestatus : '0',
            'dTaskDoneDate' => $ReminderDateTimes ? $ReminderDateTimes : 0,
            'cDoneDetails' => $donedetails ? $donedetails : "",
            'iDCRId' => 0,
            'iTaskDoneByUserId' => 0
        );
        ...
        if ($insert) {
            ...
            $UserEmailMessage = "<p>Task Assigned Date Time : $txtTaskAssignDateTime" . "</p>";
            $UserEmailMessage .= "<p>Task Assigned TO : $DepartmentName" . "</p>";
            $UserEmailMessage .= "<p>Client Req : $ClientReqName" . "</p>";
            $UserEmailMessage .= "<p>Requirement : $RequirementTitle" . "</p>";
            $UserEmailMessage .= "<p>Client Prop : $ClientPropName" . "</p>";
            $UserEmailMessage .= "<p>Property : $PropertyName" . "</p>";
            $UserEmailMessage .= "<p>Task : $TaskName" . "</p>";
            $UserEmailMessage .= "<p>Task Summary : $TaskSummary" . "</p>";
            $UserEmailMessage .= "<p>Task Target DateTime : $txtTaskTargetDateTime" . "</p>";
            $UserEmailMessage .= "<p>Task Reminder DateTime : $txtReminderDateTime" . "</p>";
            $UserEmailMessage .= "<p>Task Assigned By : $TaskAssignedByUserName" . "</p>";
            ...
            $mail->IsSMTP();                                     // We are going to use SMTP
            $mail->SMTPAuth = true;                                 // Enabled SMTP authentication
            $mail->SMTPSecure = "ssl"; //""; //"ssl";                                // Prefix for secure protocol to connect to the server
            $mail->Host = "mail.linkersindia.in";                 // Setting GMail as our SMTP server
            $mail->Port = 465;            // SMTP port to connect to GMail
            //$mail->SMTPDebug = 4;
            $mail->Username = "support@linkersindia.in";           // User email address
            $mail->Password = "sup123";
            $mail->IsHTML(true); // Password in GMail
    ...
    function edit_task_assign($data) {
        ...
        $editData = array(
            ...
            'dReminderDateTime' => $ReminderDateTime,
            'bTaskDone' => $taskdonestatus ? $taskdonestatus : '0',
            'dTaskDoneDate' => $ReminderDateTimes,
            'cDoneDetails' => $donedetails
        );
    ...
    function get_all_property_master_datatable($client_id = 0) {
        ...
        current_status_master.cCurrentStatusName,CASE property_master.bAcceptTermsAndConditions 
        WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bAcceptTermsAndConditions',
        CASE property_master.bActive WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bActive',user_master.cName,DATE_FORMAT(property_master.dDate,'%d/%m/%Y') AS dDate");

        // DB table to use
        $sIndexColumn = 'property_master.iPropertyId';
        $sTable = 'property_master';
        $where = "  property_master.bDelete=0";
      //$where = "  property_master.bDelete=0 " . $sql;
        ...
        if ($client_id != 0) {
            $where = $where . " and iClientId = " . $client_id;
        }
        ...
        if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {
            ...
            $sWhere .= " cPropertyName LIKE '%" . $this->db->escape_like_str($sSearch) . "%' 
                        OR cName LIKE '%" . $this->db->escape_like_str($sSearch) . "%'  
                         OR cCurrentStatusName LIKE '%" . $this->db->escape_like_str($sSearch) . "%' ";
//            for ( $i=0 ; $i<count($aColumns) ; $i++ )
//            {
//
...
//            $sWhere = substr_replace( $sWhere, "", -3 );
        ...
        $this->db->from($sTable);
        $this->db->join("current_status_master", "property_master.iCurrentStatusId=current_status_master.iCurrentStatusId", "LEFT");
        $this->db->join("user_master", "property_master.iAddedByUserId=user_master.iUserId", "LEFT");
        $this->db->where($sWhere, null, false);
        $rResult = $this->db->get();
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $row[6] = $Active;
            $row[7] = $staffname;
            $row[8] = $viewporturlsht;
            $row[9] = $editurls;
            $row[10] = $propertydels;
            $row[11] = $propertyhistorys;
...
//        echo '</pre>';die;
    ...
    function add_property_master($data) {
        ...
        if (!empty($ClientId)) {
            ...
            $cContactPerson1Emails = trim($row1['cContactPerson1Email']);
        ...
        if(!empty($data['cmbAvailabilityStatus'])) {
            $AvailabilityStatus = trim($data['cmbAvailabilityStatus']);
        } else {
            $AvailabilityStatus = "";
        }
        ...
        if ($query) {
            if ($query->num_rows > 0) {
                ...
                $addData = array(
                    ...
                    'iClientId' => $ClientId ? $ClientId : 0,
                    'iContactId' => $ContactId ? $ContactId : 0,
                    'iBranchId' => $BranchId ? $BranchId : 0,
                    'iStateId' => $StateId ? $StateId : 0,
                    'iDistrictId' => $DistrictId ? $DistrictId : 0,
                    'iCityId' => $CityId ? $CityId : 0,
                    'iLocationId' => $LocationId ? $LocationId : 0,
                    'cPropertyAddress' => htmlentities($PropertyAddress, ENT_QUOTES),
                    'iSourceId' => $SourceId ? $SourceId : 0,
                    'iPropertyCategoryId' => $PropertyCategoryId ? $PropertyCategoryId : 0,
                    'iPropertyTypeId' => $PropertyTypeId ? $PropertyTypeId : 0,
                    'iPropertyStatusId' => $PropertyStatusId ? $PropertyStatusId : 0,
                    'cPropertyPurpose' => $PropertyPurpose ? $PropertyPurpose : "",
                    'cPropertyLegalStatus' => $PropertyLegalStatus,
                    'cAvailabilityStatus' => $AvailabilityStatus,
                    ...
                    'iNoOfFloorsInBuilding' => $NoOfFloorsInBuilding ? $NoOfFloorsInBuilding : 0,
                    'cGroundCoverage' => htmlentities($GroundCoverage, ENT_QUOTES),
                    'cFloorOffered' => $FloorOffered ? $FloorOffered : 0,
                    ...
                    'iAgreementTypeId' => $AgreementTypeId ? $AgreementTypeId : 0,
                    ...
                    'dLeaseUpToDate' => $LeaseUpToDate ? $LeaseUpToDate : 0,
                    'cPreviousTenant' => htmlentities($PreviousTenant, ENT_QUOTES),
                    'dAgreementDate' => $AgreementDate ? $AgreementDate : 0,
                    'cAgreementPlace' => $AgreementPlace ? $AgreementPlace : 0,
                ...
                $psradded .= "<p>Greetings from Linkers India!</p>
                                <p>We are pleased to share that we have added your Property Specification in our CRM.<br>We strive to deliver quality Corporate Leasing Services and assure you that we shall soon get back with suitable options.</p>
                                <p>Thanks!</p>
                                 <p>Best Regards,<br>";

                $psradded .= "LINKERS INDIA<br>211, Bakhatgarh Tower, 10/1, New Palasiya, Indore-452001<br>
            Ph. 0731-4044406, Cell: 8349998444, Email: <a href='mailto:support@linkersindia.in' target='_blank'>support@linkersindia.in</a>
                        </p></div>";

                //$this->sendEmail($cContactPerson1Emails, 'PSR is added', $psradded);
               // $cContactPerson1Emailsmail = $data['dvEmaildata'] ? $data['dvEmaildata'] : $cContactPerson1Emails;
               // $this->sendEmail($cContactPerson1Emailsmail, 'PSR is added', $psradded);
                  
                $selectedClientEmail = trim($data['dvEmaildata']);
                
                if(!empty($selectedClientEmail)){
                  $this->sendEmail($selectedClientEmail, 'PSR is added', $psradded);                         
                }                           
                ...
                $psraddInternalStaff .= "<p>Best Regards,<br>";

                $psraddInternalStaff .= "LINKERS INDIA<br>211, Bakhatgarh Tower, 10/1, New Palasiya, Indore-452001<br>
            Ph. 0731-4044406, Cell: 8349998444, Email: <a href='mailto:support@linkersindia.in' target='_blank'>support@linkersindia.in</a>
                        </p></div>";
                ...
                if ($insert) {
                    ...
                    foreach ($data['hfAttachmentFileName'] as $key => $item) {
                        ...
                        if ($item != "") {
                            $AttatchData['iPropertyId'] = $propertys_insert_id;
    ...
    function dealdone_existing_attachment_delete($AttachmentId, $AttachmentPath) {   // file not remove from folder & db, just update its db status to be deleted.									
        $this->db->where('iDealAttachmentId', $AttachmentId);
        ...
        if ($query) {
            if ($query->num_rows() > 0) {
                ...
                $this->db->where('iDealAttachmentId', $AttachmentId);
                $delsql = $this->db->update('deal_attachments', $delarray);
    ...
    function get_all_requirement_master_datatable($client_id = 0) {
        /* $sql = "SELECT requirement_master.iRequirementId,requirement_master.dDate
          ,requirement_master.cRequirementTitle,client_master.cClientName
          ,current_status_master.cCurrentStatusName,CASE requirement_master.bAcceptTermsAndConditions WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bAcceptTermsAndConditions'
          ,CASE requirement_master.bActive WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bActive'
		...
		,requirement_master.cRequirementTitle,client_master.cClientName
		,current_status_master.cCurrentStatusName,CASE requirement_master.bAcceptTermsAndConditions WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bAcceptTermsAndConditions'
		,CASE requirement_master.bActive WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bActive',user_master.cName");
        ...
        if($client_id != 0) {
            $where = $where . " and requirement_master.iClientId = " . $client_id;
        }
...
//            $sWhere = substr_replace( $sWhere, "", -3 );
        ...
        $this->db->join("user_master", "requirement_master.iAddedByUserId=user_master.iUserID", "LEFT");
        $this->db->where($sWhere, null, false);
        $rResult = $this->db->get();
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $inspectionsUrl = base_url() . 'index.php/dashboard/requisitionhistory_inspections/' . $RequirementId;
            $inspectionsCount = '<a class="various" data-fancybox-type="iframe" href="' . $inspectionsUrl . '" title="Inspections Count" target="_blank" title="Inspections Count">'. (!empty($this->requisitionhistory_inspections($RequirementId)) ? count($this->requisitionhistory_inspections($RequirementId)) : 0) .'</a>';

            $deliveriesUrl = base_url() . 'index.php/dashboard/requisitionhistory_deliveries/' . $RequirementId;
            $deliveriesCount = '<a class="various" data-fancybox-type="iframe" href="' . $deliveriesUrl . '" title="Deliveries Count" target="_blank" title="Deliveries Count">'. (!empty($this->requisitionhistory_deliveries($RequirementId)) ? count($this->requisitionhistory_deliveries($RequirementId)) : 0) .'</a>';

            $row[0] = $RequirementId;
            //$row[1] = $Date;
            $row[1] = $RequirementTitle;
            $row[2] = $CurrentStatusName;
            $row[3] = $AcceptTermsAndConditions;
            $row[4] = $Active;
            $row[5] = $staffname;
            $row[6] = $inspectionsCount;
            $row[7] = $deliveriesCount;
            $row[8] = $viewporturlsht;
            $row[9] = $editurls;
            $row[10] = $propertyhistorys;
    ...
    function add_requirement_master($data) {
        ...
        if ($query) {
            if ($query->num_rows > 0) {
                ...
                $addData = array(
                    'dDate' => $Date,
                    'iAddedByUserId' => $AddedByUserId ? $AddedByUserId : 0,
                    ...
                    'iBranchId' => (int) $BranchId ? $BranchId : 0,
                    'iStateId' => (int) $StateId ? $StateId : 0,
                    'iDistrictId' => (int) $DistrictId ? $DistrictId : 0,
                    'iCityId' => (int) $CityId ? $CityId : 0,
                    'iLocationId' => (int) $LocationId ? $LocationId : 0,
                    'cArea' => $Area ? mysql_real_escape_string($Area) : "",
                    'cHeight' => $Height ? htmlentities($Height, ENT_QUOTES) : 0,
                    'cFrontage' => $Frontage ? htmlentities($Frontage, ENT_QUOTES) : 0,
                    'cFurnishedStatus' => htmlentities($cmbFurnishedStatus, ENT_QUOTES),
                    'iSourceId' => (int) $SourceId ? $SourceId : 0,
                    //'cPropertyPurpose' => $PropertyPurpose,
                    'iBusinessPurposeId' => (int) $BusinessPurposeId ? $BusinessPurposeId : 0,
                    'iPropertyCategoryId' => (int) $PropertyCategoryId ? $PropertyCategoryId : 0,
                    'cRequirementType' => $RequirementType ? $RequirementType : "",
                    'cBudgetPerMonth' => $BudgetPerMonth,
                    'cFloorLevelPreference' => $FloorLevelPreference,
                    'iEscalationId' => (int) $EscalationId ? $EscalationId : 0,
                    ...
                    'dAgreementDate' => $AgreementDate,
                    'cAgreementPlace' => $AgreementPlace,
                    'cPerson1DuringAgreement' => $Person1DuringAgreement ? $Person1DuringAgreement : "",
                    'cPerson2DuringAgreement' => $Person2DuringAgreement ? $Person2DuringAgreement : "",
                    'cAgreementFilePath' => $AgreementFilePath ? $AgreementFilePath : "",
                    'cAgreementFileName' => $AgreementFileName ? $AgreementFileName : "",
                    'cTermsAndConditions' => htmlentities($TermsAndConditions, ENT_QUOTES),
                    'bAcceptTermsAndConditions' => $AcceptTermsAndConditions ? $AcceptTermsAndConditions : 0,
                ...
                $Rfaddtointernalstaff .= "<p>Best Regards,<br>";
                $Rfaddtointernalstaff .= "LINKERS INDIA<br>211, Bakhatgarh Tower, 10/1, New Palasiya, Indore-452001<br>
            Ph. 0731-4044406, Cell: 8349998444, Email: <a href='mailto:support@linkersindia.in' target='_blank'>support@linkersindia.in</a>
                        </p></div>";
                ...
                if ($insert) {
                    ...
                    $RFaddmail .= "<p>Greetings from Linkers India!</p>
                                    <p>We are pleased to share that we have added your Property Requirement in our CRM.<br>We strive to deliver quality Corporate Leasing Services and assure you that we shall soon get back with suitable options.</p>
                                    <p>Thanks!</p>
                                    <p>Best Regards,<br>";
                    $RFaddmail .= "LINKERS INDIA<br>211, Bakhatgarh Tower, 10/1, New Palasiya, Indore-452001<br>
            Ph. 0731-4044406, Cell: 8349998444, Email: <a href='mailto:support@linkersindia.in' target='_blank'>support@linkersindia.in</a>
                        </p></div>";

                    //$this->sendEmail($cContactPerson1Email, 'New RF Added', $RFaddmail);

                    //$cContactPerson1Emailsmail = $data['dvEmaildata'] ? $data['dvEmaildata'] : $cContactPerson1Email;

                    //$this->sendEmail($cContactPerson1Emailsmail, 'New RF Added', $RFaddmail);

                      $selectedClientEmail = trim($data['dvEmaildata']);

                      if(!empty($selectedClientEmail)){
                          $this->sendEmail($selectedClientEmail, 'New RF Added', $RFaddmail);                          
                      }  
        ...
        if ($query) {
            if ($query->num_rows > 0) {
                ...
                $editData = array(
                    ...
                    'cArea' => mysql_real_escape_string($Area),
                    'cHeight' => htmlentities($Height, ENT_QUOTES),
                    'cFrontage' => htmlentities($Frontage, ENT_QUOTES),
                    'cFurnishedStatus' => htmlentities($cmbFurnishedStatus, ENT_QUOTES),
    ...
    function add_deal_initiate($data) {
      //$UserId = trim($this->session->userdata('UserId'));
        ...
        if ($insert) {
            ...
            if (isset($data['hfAttachmentPath'])) {


                $AttatchData['cAttachmentTitle'] = $data['txtAttachmentTitle'][0];

                $AttatchData['cAttachmentPath'] = $data['hfAttachmentPath'][0];

                $AttatchData['cAttachmentName'] = $data['hfAttachmentName'][0];
    ...
    function edit_deal_initiate($data) {
        ...
        if ($update) {
            ...
            if (isset($data['hfAttachmentPath'])) {
                ...
                $AttatchData['iDealInitiateId'] = $hfDealInitiateId;

                if ($AttatchData['cAttachmentPath'] != "") {
                    $this->db->delete('deal_attachments', array('iDealInitiateId' => $hfDealInitiateId));
                    $insertAttatchData = $this->db->insert('deal_attachments', $AttatchData);
                }
...
WHERE deal_lost.bDelete=0";
    ...
    function client_history_print_report($ClientId) {
        ...
        $sql = "SELECT (select cClientName from client_master where client_master.iClientId = dcr_detail.clientid) as clientsname,dcr_summary.dDCRDate,user_master.cName,dcr_summary.cDCRRemarks,
                task_master.cTaskName,property_master.cPropertyName,requirement_master.cRequirementTitle,
                CMREQ.cClientName as cReqClientName,CMPROP.cClientName as cPropClientName,
                dcr_detail.cDCRSummary FROM dcr_detail LEFT JOIN dcr_summary ON dcr_detail.iDCRId=dcr_summary.iDCRId 
                LEFT JOIN user_master ON dcr_summary.iUserId=user_master.iUserId 
                LEFT JOIN task_master ON dcr_detail.iTaskId=task_master.iTaskId 
                LEFT JOIN property_master ON dcr_detail.iPropertyId=property_master.iPropertyId 
                LEFT JOIN requirement_master ON dcr_detail.iRequirementId=requirement_master.iRequirementId 
		LEFT JOIN client_master as CMREQ ON dcr_detail.iClientReqId=CMREQ.iClientId 
                LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId=CMPROP.iClientId 
                WHERE (dcr_detail.iClientReqId='" . $ClientId . "' OR dcr_detail.iClientPropId='" . $ClientId . "' OR dcr_detail.clientid='" . $ClientId . "')  
                AND dcr_detail.bDelete=0 ORDER BY dcr_summary.dDCRDate ASC";
        ...
        $tbl .= "<tr style=\"background-color:#CCCCCC;\">
				...
				<td width=\"10%\" align=\"left\"><b>Client</b></td>
        ...
        if ($query) {
            if ($query->num_rows() > 0) {
                ...
                foreach ($query->result_array() as $row) {
                    ...
                    if ($row['clientsname'] != '') {
                        $clientsname = trim($row['clientsname']);
                    } else {
                        $clientsname = "";
                    }

                    if ($clientsname != "") {
                        $clientsnames = $clientsname;
                    } else {
                        $clientsnames = $ReqClientName;
                    }
                    ...
                    $tbl .= "<td align=\"left\">" . $clientsnames . "</td>";
    ...
    function property_history_print_report($PropertyId) {
        ...
        $sql = "SELECT (select cClientName from client_master where client_master.iClientId = dcr_detail.clientid) as clientsname, dcr_summary.dDCRDate,user_master.cName,dcr_summary.cDCRRemarks,task_master.cTaskName,requirement_master.cRequirementTitle,property_master.cPropertyName,CMREQ.cClientName as cReqClientName,CMPROP.cClientName as cPropClientName,dcr_detail.iPropertyId,dcr_detail.cDCRSummary
...
//		$pdf->SetHeaderData(PDF_HEADER_LOGO, $PDF_HEADER_LOGO_WIDTH, $PDF_HEADER_TITLE, $PDF_HEADER_STRING);
        ...
        $pdf->SetFont('helvetica', 'B', 15);
        $pdf->MultiCell(100, 5, $AttachmentTitles22 . $AttachmentTitle33
                , $border = 0, $align = 'L', $fill = false, $ln = 1, $x = '75', $y = '', $reseth = true, $stretch = 0, $ishtml = true, $autopadding = true, $maxh = 0, $valign = 'T', $fitcell = false);


        $AttachmentTitle323 = ' support@linkersindia.in<br>Ph: 0731-4044406<br>Cell: 8349998444 ';

        $AttachmentTitles22s = wordwrap($AttachmentTitle323, 21, "<br>\n", TRUE);

        $pdf->SetFont('helvetica', '', 11);
        $pdf->MultiCell(44, 5, $AttachmentTitle323
                , $border = 0, $align = 'L', $fill = false, $ln = 1, $x = '160', $y = '', $reseth = true, $stretch = 0, $ishtml = true, $autopadding = true, $maxh = 0, $valign = 'T', $fitcell = false);
        $pdf->ln(10);
...
//                echo '</pre>';die;
                ...
                if (!empty($row['cHeight'])) {
                    $Heights = trim($row['cHeight']);
                    $Height1 = str_replace("ft", "ft\n", $Heights);
                    $Height = str_replace("Ft", "Ft\n", $Height1);
                    $Height = trim($Height);
                } else {
                    $Height = "";
                }
...
//                }
                ...
                if (!empty($AgreementTypeName) || !empty($DemandPerSqFeet) || !empty($DemandGross) || !empty($CAM)) {
...
//				$tbl.="</tr>";
        ...
        if (isset($AttachmentRows) && count($AttachmentRows) > 0) {
            ...
            foreach ($AttachmentRows as $docrow) {
                ...
                $pdf->Cell(60, 5, $AttachmentTitle, $border = 0, $ln = 0, $align = 'L', $fill = false, $link = '', $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'M');
                $pdf->Cell(130, 5, "Click to view Image", $border = 0, $ln = 0, $align = 'L', $fill = false, $link = $ImgUrls, $stretch = 0, $ignore_min_height = false, $calign = 'T', $valign = 'M');
                // $pdf->MultiCell(130, 5, "<a href='".."' target=''>Click to view Image</a>", $border = 0, $align = 'L', $fill = false, $ln = 1, $x = '', $y = '', $reseth = true, $stretch = 0, $ishtml = true, $autopadding = true, $maxh = 0, $valign = 'T', $fitcell = false);

                // $pdf->Image($ImgUrls, 76, $pdf->gety(), 58, 38, $type = '', $link = $ImgUrls, $align = '', $resize = false, $dpi = 200, $palign = '', $ismask = false, $imgmask = false, $border = 0, $fitbox = false, $hidden = false, $fitonpage = false, $alt = false, $altimgs = array());
				// $pdf->MultiCell(60, 5, $AttachmentTitle, $border = 0, $align = 'L', $fill = false, $ln = 1, $x = '', $y = '', $reseth = true, $stretch = 0, $ishtml = true, $autopadding = true, $maxh = 0, $valign = 'T', $fitcell = false);
    ...
    function requisition_history_print_report($RequirementId) {
        ...
        $sql = "SELECT (select cClientName from client_master where client_master.iClientId = dcr_detail.clientid) as clientsname, dcr_summary.dDCRDate,user_master.cName,dcr_summary.cDCRRemarks,task_master.cTaskName,requirement_master.cRequirementTitle,CMREQ.cClientName as cReqClientName,property_master.cPropertyName,CMPROP.cClientName as cPropClientName,dcr_detail.iRequirementId,dcr_detail.cDCRSummary
				FROM dcr_detail LEFT JOIN dcr_summary ON dcr_detail.iDCRId=dcr_summary.iDCRId LEFT JOIN user_master ON dcr_summary.iUserId=user_master.iUserId LEFT JOIN task_master ON dcr_detail.iTaskId=task_master.iTaskId LEFT JOIN requirement_master ON dcr_detail.iRequirementId=requirement_master.iRequirementId LEFT JOIN property_master ON dcr_detail.iPropertyId=property_master.iPropertyId
    ...
    function dashboard_datewise_dcr_print_report($Date) {


        $typeid = trim($this->input->get('typeid'));
        ...
        $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
        $tbl .= "<tr>
				  <td align=\"center\" style=\"text-align:center; font-size:45px;text-decoration:underline;\"><strong>Today's Statistics</strong></td>
			  </tr>
			  <tr>
				  <td>&nbsp;</td>
			  </tr>
			  <tr>
				  <td align=\"center\" style=\"text-align:center; font-size:45px;text-decoration:underline;\"><strong>Date : $DCRDateid</strong></td>
			  </tr>
			  <tr>
				<td>&nbsp;</td>
			  </tr>";
        ...
        $sql77 = "SELECT dcr_summary.iDCRId,user_master.cName FROM dcr_summary LEFT JOIN user_master ON dcr_summary.iUserId=user_master.iUserId 
                  LEFT JOIN dcr_detail ON dcr_detail.iDCRId=dcr_summary.iDCRId 
                  WHERE dcr_summary.dDCRDate='" . $DCRDate . "' AND dcr_summary.bDelete=0 AND dcr_detail.iTaskId = '" . $taskTypes . "' ";
        ...
        if ($query77) {
            if ($query77->num_rows() > 0) {
                ...
                $dataunique1 = $query77->result_array();

                $createnew_array = array();
                foreach ($dataunique1 as $dataunique1value) {
                    $names = $dataunique1value['cName'];

                    if (!in_array($names, $dataunique1value['cName'])) {
                        $createnew_array[$names][] = $dataunique1value['iDCRId'];
                    }
                }
                ...
                foreach ($createnew_array as $keys => $row77) {
                    $skeyss = 0;
                    ...
                    $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">
									...
									<tr style=\"background-color:#CCCCCC;\">
									 <td width=\"10%\" align=\"left\"><b>&nbsp;Sno.</b></td>
									 <td width=\"10%\" align=\"left\"><b>Task</b></td>
									 <td width=\"17%\" align=\"left\"><b>Client</b></td>
                    ...
                    foreach ($row77 as $row777) {
                        ...
                        if ($taskTypes != "" && $taskTypes != 0) {
                            $sql7 .= " AND task_master.iTaskId = '" . $taskTypes . "' ";
                        }
                        ...
                        if ($query7) {
                            if ($query7->num_rows() > 0) {
                                ...
                                foreach ($query7->result_array() as $incKey => $row7) {
...
//	  exit;
    ...
    function pending_rf_print_report($FromDate, $ToDate) {
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $splitfrmdt = explode('/', $FromDate);
        $FromDates = $splitfrmdt[2] . "-" . $splitfrmdt[1] . "-" . $splitfrmdt[0];

        $splittodt = explode('/', $ToDate);
        $ToDates = $splittodt[2] . "-" . $splittodt[1] . "-" . $splittodt[0];

        $sql = "SELECT requirement_master.cRequirementTitle,requirement_master.dDate as dAddedOnDate,
            client_master.cClientName,user_master.cName as cAddedByStaffName,
            current_status_master.cCurrentStatusName FROM requirement_master 
            LEFT JOIN client_master ON requirement_master.iClientId=client_master.iClientId 
            LEFT JOIN user_master ON requirement_master.iAddedByUserId=user_master.iUserId 
            LEFT JOIN current_status_master ON requirement_master.iCurrentStatusId=current_status_master.iCurrentStatusId 
            WHERE requirement_master.bActive=1 AND 
            requirement_master.dDate BETWEEN '" . $FromDates . "' AND '" . $ToDates . "' 
            AND requirement_master.bDelete=0 ORDER BY requirement_master.dDate DESC";
    ...
    function datewise_weekly_print_report($FromDt, $ToDt) {
            ...
            if ($query2->num_rows() > 0) {
                ...
                foreach ($query2->result_array() as $row2) {
                    ...
                    foreach ($query1->result_array() as $row1) {
                        ...
                        $tbl .= "<td style=\"width:" . $xcolwidth . "%; height:25px; text-align:center;\">" .
                                '<a  href="' . base_url() . 'index.php/dashboard/drilldown_report/?taskid=' . $TaskId . '&user_id=' . $UserId . '&formdate=' . $FromDate . '&todate=' . $ToDate . '" style="text-decoration:none;" target="_blank">' . $CountTask . '</a>'
                                . "</td>";

                        //href="#" onclick='window.open("'.base_url().'index.php/dashboard/drilldown_report/?taskid='.$TaskId.'&user_id='.$UserId.'&formdate='.$FromDate.'&todate='.$ToDate.'");return false;'
        ...
        //Close and output PDF document
        return $pdf->Output('datewise_dcr_report.pdf', 'I');
        //return $pdf->Output('datewise_dcr_report.pdf', 'FD');
        //============================================================+
        // END OF FILE
        //============================================================+	
    }

    function rf_pending_acceptance_print_report($FromDate, $ToDate) {
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $sql = "SELECT requirement_master.cRequirementTitle,requirement_master.dDate as dAddedOnDate,client_master.cClientName,user_master.cName as cAddedByStaffName,current_status_master.cCurrentStatusName FROM requirement_master LEFT JOIN client_master ON requirement_master.iClientId=client_master.iClientId LEFT JOIN user_master ON requirement_master.iAddedByUserId=user_master.iUserId LEFT JOIN current_status_master ON requirement_master.iCurrentStatusId=current_status_master.iCurrentStatusId WHERE requirement_master.bAcceptTermsAndConditions=0 AND requirement_master.bActive=1 AND requirement_master.dDate BETWEEN '" . $FromDates . "' AND '" . $ToDates . "' AND requirement_master.bDelete=0 ORDER BY requirement_master.dDate DESC";
    ...
    function psr_pending_acceptance_print_report($FromDate, $ToDate) {
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    function available_properties_print_report($FromDate, $ToDate) {
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $sql = "SELECT property_master.cPropertyName,property_master.dDate as dAddedOnDate,client_master.cClientName,user_master.cName as cAddedByStaffName,current_status_master.cCurrentStatusName FROM property_master LEFT JOIN client_master ON property_master.iClientId=client_master.iClientId LEFT JOIN user_master ON property_master.iAddedByUserId=user_master.iUserId LEFT JOIN current_status_master ON property_master.iCurrentStatusId=current_status_master.iCurrentStatusId WHERE property_master.bActive=1 AND  property_master.dDate BETWEEN '" . $FromDates . "' AND '" . $ToDates . "' AND property_master.bDelete=0 ORDER BY property_master.dDate DESC";
    ...
    function property_search_print_report($StateId, $CityId, $PropertyCategoryId, $PropertyTypeId) {
        ...
        $sql1 = "SELECT property_master.cAvailabilityStatus,property_master.cPropertyName,property_master.dDate as dAddedOnDate,client_master.cClientName,user_master.cName as cAddedByStaffName,current_status_master.cCurrentStatusName FROM property_master LEFT JOIN client_master ON property_master.iClientId=client_master.iClientId LEFT JOIN user_master ON property_master.iAddedByUserId=user_master.iUserId LEFT JOIN current_status_master ON property_master.iCurrentStatusId=current_status_master.iCurrentStatusId";
        ...
        $tbl .= "<tr>
			   ...
			   <tr style=\"background-color:#CCCCCC; font-size:38px;\">
				<td width=\"5%\" align=\"center\"><b>SNo.</b></td> 
				<td width=\"20%\" align=\"left\"><b>Client</b></td>
				<td width=\"18%\" align=\"left\"><b>PSR Title</b></td>
				<td width=\"15%\" align=\"center\"><b>Added On</b></td>
				<td width=\"15%\" align=\"left\"><b>Added By</b></td>
                <td width=\"18%\" align=\"left\"><b>Current Status</b></td>
				<td width=\"15%\" align=\"left\"><b>Availablity Status</b></td>
			    </tr>
			    <tr>
				   <td colspan=\"6\">&nbsp;</td>
			   </tr>";
        ...
        if ($query) {
            if ($query->num_rows() > 0) {
                ...
                foreach ($query->result_array() as $row) {
                    ...
                    if ($row['cAvailabilityStatus'] != '') {
                        $cAvailabilityStatus = trim($row['cAvailabilityStatus']);
                    } else {
                        $cAvailabilityStatus = "NA";
                    }
                    ...
                    $tbl .= "<td align=\"left\">" . $AddedByStaffName . "</td>";
                    $tbl .= "<td align=\"left\">" . $CurrentStatusName . "</td>";
                    $tbl .= "<td align=\"left\">" . $cAvailabilityStatus . "</td>";
...
//            
    ...
    function requirement_aging_report($FromDate, $ToDate) {
//       // error_reporting(E_ALL);
        ...
        /* $sql = "SELECT requirement_master.cRequirementTitle,requirement_master.dDate as dAddedOnDate,
          ...
          LEFT JOIN client_master ON requirement_master.iClientId=client_master.iClientId
          LEFT JOIN user_master ON requirement_master.iAddedByUserId=user_master.iUserId
          LEFT JOIN current_status_master ON
          requirement_master.iCurrentStatusId=current_status_master.iCurrentStatusId
          WHERE requirement_master.bActive=1 AND requirement_master.bDelete=0
          ORDER BY requirement_master.dDate ASC "; */
        ...
        $sql = "SELECT dcr_summary.dDCRDate as dAddedOnDate,user_master.cName,task_master.cTaskName,
                property_master.cPropertyName,requirement_master.cRequirementTitle,
                CMREQ.cClientName as cReqClientName,CMPROP.cClientName as cPropClientName 
                FROM dcr_detail LEFT JOIN dcr_summary ON dcr_detail.iDCRId=dcr_summary.iDCRId 
                LEFT JOIN user_master ON dcr_summary.iUserId=user_master.iUserId 
                LEFT JOIN task_master ON dcr_detail.iTaskId=task_master.iTaskId 
                LEFT JOIN property_master ON dcr_detail.iPropertyId=property_master.iPropertyId 
                LEFT JOIN requirement_master ON dcr_detail.iRequirementId=requirement_master.iRequirementId 
                LEFT JOIN client_master as CMREQ ON dcr_detail.iClientReqId=CMREQ.iClientId 
                LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId=CMPROP.iClientId 
                WHERE dcr_detail.bDelete=0 AND dcr_summary.dDCRDate<>'0000-00-00' 
                AND requirement_master.cRequirementTitle<>''  AND 
                dcr_summary.dDCRDate BETWEEN '" . $FromDates . "' AND '" . $ToDates . "' 
                ORDER BY dcr_summary.dDCRDate ASC ";
        ...
        $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
        $tbl .= "<tr>
				   <td colspan=\"6\" align=\"center\" style=\"text-align:center; font-size:43px;\"><strong>Requirement Aging Report</strong></td>
			   </tr>
			   <tr>
				   <td colspan=\"6\">&nbsp;</td>
			   </tr>
			   <tr style=\"background-color:#CCCCCC; font-size:38px;\">
				...
				<td width=\"40%\" align=\"left\"><b>Requirement</b></td>
            ...
            if ($query->num_rows() > 0) {
                ...
                foreach ($query->result_array() as $row) {
                    ...
                    if ($row['dAddedOnDate'] != '') {
                        ...
                        $datetime2 = new DateTime($AddedOnDt);
                        $interval = $datetime1->diff($datetime2);
                        $datediff = $interval->format('%R%a days');

                        $datediffstr = str_replace('-', '', $datediff);
                    } else {
                        $AddedOnDate = "";
                        $datediff = "";
                    }
                    ...
                    $tbl .= "<td align=\"left\">" . $RequirementTitle . "</td>";
                    $tbl .= "<td align=\"center\">" . $AddedOnDate . "</td>";
                    $tbl .= "<td align=\"left\">" . $datediffstr . "</td>";
                    //$tbl.="<td align=\"left\">" . $CurrentStatusName . "</td>";
        ...
        //Close and output PDF document
        return $pdf->Output('rf_open_report.pdf', 'I');
    }

    function property_aging_report($FromDate, $ToDate) {
//       // error_reporting(E_ALL);
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $sql = "SELECT dcr_summary.dDCRDate as dAddedOnDate,user_master.cName,task_master.cTaskName,
                ...
                LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId=CMPROP.iClientId 
                WHERE dcr_detail.bDelete=0 AND dcr_summary.dDCRDate<>'0000-00-00' 
                AND  dcr_summary.dDCRDate BETWEEN '" . $FromDates . "' AND '" . $ToDates . "' 
                AND requirement_master.cRequirementTitle<>'' ORDER BY dcr_summary.dDCRDate ASC ";
        ...
        $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
        $tbl .= "<tr>
				   <td colspan=\"6\" align=\"center\" style=\"text-align:center; font-size:43px;\"><strong>Property Aging Report</strong></td>
			   </tr>
			   <tr>
				   <td colspan=\"6\">&nbsp;</td>
			   </tr>
			   <tr style=\"background-color:#CCCCCC; font-size:38px;\">
				<td width=\"5%\" align=\"center\"><b>SNo.</b></td> 
                <td width=\"10%\" align=\"center\">&nbsp;</td> 
				<td width=\"40%\" align=\"left\"><b>Property</b></td>
				<td width=\"25%\" align=\"center\"><b>Date</b></td>
                <td width=\"10%\" align=\"center\"><b>Days</b></td>
			    </tr>
			    <tr>
        ...
        if ($query) {
            if ($query->num_rows() > 0) {
                ...
                foreach ($query->result_array() as $row) {
                    ...
                    $tbl .= "<td align=\"center\"></td>";
                    //  $tbl.="<td align=\"left\">" . $ClientName . "</td>";
                    $tbl .= "<td align=\"left\">" . $cPropertyName . "</td>";
                    $tbl .= "<td align=\"center\">" . $AddedOnDate . "</td>";
                    $tbl .= "<td align=\"left\">" . $datediffstr . "</td>";
                    //$tbl.="<td align=\"left\">" . $CurrentStatusName . "</td>";
        ...
        //Close and output PDF document
        return $pdf->Output('psr_open_report.pdf', 'I');
    }

    function property_available_report() {
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $sql = "SELECT property_master.iPropertyId,property_master.dDate,property_master.cPropertyName, 
                current_status_master.cCurrentStatusName FROM property_master 
                LEFT JOIN current_status_master ON property_master.iCurrentStatusId=current_status_master.iCurrentStatusId 
                WHERE property_master.bDelete=0 ORDER BY property_master.iPropertyId DESC ";
        ...
        $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
        $tbl .= "<tr>
				   <td colspan=\"6\" align=\"center\" style=\"text-align:center; font-size:43px;\"><strong>Property Available Report</strong></td>
			   </tr>
			   <tr>
				   <td colspan=\"6\">&nbsp;</td>
			   </tr>
			   <tr style=\"background-color:#CCCCCC; font-size:38px;\">
				<td width=\"5%\" align=\"center\"><b>SNo.</b></td> 
                                <td width=\"10%\" align=\"center\">&nbsp;</td> 
				<td width=\"70%\" align=\"left\"><b>Property Name</b></td>
        ...
        //Close and output PDF document
        return $pdf->Output('property_available_report.pdf', 'I');
    }

    function residential_property_report() {
        // create new PDF document
        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
        ...
        $sql = "SELECT property_master.iPropertyId,property_master.dDate,property_master.cPropertyName, 
                current_status_master.cCurrentStatusName FROM property_master 
                LEFT JOIN current_status_master ON property_master.iCurrentStatusId=current_status_master.iCurrentStatusId 
                WHERE property_master.bDelete=0 AND property_master.iPropertyTypeId=2 ORDER BY property_master.iPropertyId DESC ";
        ...
        $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
        $tbl .= "<tr>
				   <td colspan=\"6\" align=\"center\" style=\"text-align:center; font-size:43px;\"><strong>Residential Property Report</strong></td>
			   </tr>
			   <tr>
				   <td colspan=\"6\">&nbsp;</td>
			   </tr>
    ...
    function need_sourcewise_reports($cmbSource, $txtDate, $cmbFilter, $txtToDate) {

        $splitdt = explode('/', $txtDate);
        $DCRDate = $splitdt[2] . "-" . $splitdt[1] . "-" . $splitdt[0];

        $splitdt2 = explode('/', $txtToDate);
        $DCRDateto = $splitdt2[2] . "-" . $splitdt2[1] . "-" . $splitdt2[0];


        if ($cmbFilter == 1) {
            $sql = "select requirement_master.dDate,
                requirement_master.cRequirementTitle From  requirement_master Where requirement_master.iSourceId='" . $cmbSource . "' AND requirement_master.dDate BETWEEN '" . $DCRDate . "' AND '" . $DCRDateto . "'  ORDER By iRequirementId DESC";
        } elseif ($cmbFilter == 2) {

            $sql = "select client_master.dCreateDate,
                client_master.cClientName From  client_master Where client_master.iSourceId='" . $cmbSource . "' AND client_master.dCreateDate BETWEEN '" . $DCRDate . "' AND '" . $DCRDateto . "'  ORDER By iClientId DESC";
        } elseif ($cmbFilter == 3) {
            $sql = "select property_master.dDate,
                property_master.cPropertyName From  property_master Where property_master.iSourceId='" . $cmbSource . "' AND property_master.dDate BETWEEN '" . $DCRDate . "' AND '" . $DCRDateto . "'  ORDER By iPropertyId DESC";
        }
        ...
        $tbl .= "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
        $tbl .= "<tr>
				   <td colspan=\"6\" align=\"center\" style=\"text-align:center; font-size:43px;\"><strong>Source Wise Report</strong></td>
			   </tr>
			   <tr>
				   <td colspan=\"6\">&nbsp;</td>
			   </tr>
        ...
        $tbl .= "<td width=\"15%\" align=\"left\"><b>Date</b></td>";

        if ($cmbFilter == 1) {

            $tbl .= "<td width=\"70%\" align=\"left\"><b>RF</b></td>";
        } elseif ($cmbFilter == 2) {
            $tbl .= "<td width=\"70%\" align=\"left\"><b>Client</b></td>";
        } elseif ($cmbFilter == 3) {
            $tbl .= "<td width=\"70%\" align=\"left\"><b>PSR</b></td>";
        }
            ...
            if ($query->num_rows() > 0) {
                ...
                foreach ($query->result_array() as $row) {
                    ...
                    if ($cmbFilter == 1) {
                        ...
                        if ($row['cRequirementTitle'] != '') {
                            $cPropertyName = trim($row['cRequirementTitle']);
                        } else {
                            $cPropertyName = "";
                        }
                        ...
                        if ($row['cClientName'] != '') {
                            $cPropertyName = trim($row['cClientName']);
                        } else {
                            $cPropertyName = "";
                        }
                        ...
                        if ($row['dCreateDate'] != '') {
                            $AddedOnDt = trim($row['dCreateDate']);
                            $splitaddondt = explode('-', $AddedOnDt);
                            $AddedOnDate = $splitaddondt[2] . "/" . $splitaddondt[1] . "/" . $splitaddondt[0];
                        } else {
                            $AddedOnDate = "";
                    ...
                    $tbl .= "<td align=\"center\"></td>";
                    //  $tbl.="<td align=\"left\">" . $ClientName . "</td>";
                    $tbl .= "<td align=\"left\">" . $AddedOnDate . "</td>";
        ...
        //Close and output PDF document
        return $pdf->Output('source_wise_report.pdf', 'I');
    }

    function getClientName($id) {
        $sqlemail = "SELECT cClientName FROM client_master WHERE iClientId = '" . $id . "'";
        $queryemail = $this->db->query($sqlemail);
        if ($queryemail) {
            $res = $queryemail->row_array();
            return $res['cClientName'];
        } else {
            return false;
        }
    }
    ...
    function getUserName($userid) {
        $sql = "SELECT cName FROM user_master WHERE user_master.iUserId='".$userid."'";
        $query = $this->db->query($sql);
        if ($query) {
            $res = $query->row_array();
            return $res['cName'];
    ...
    function getTaskName($taskid) {
        $sql = "SELECT cTaskName FROM task_master WHERE task_master.iTaskId='".$taskid."'";
        $query = $this->db->query($sql);
        if ($query) {
            $res = $query->row_array();
            return $res['cTaskName'];
        } else {
            return false;
        }
    }

    function get_drill_down_datatable($taskid, $userid, $formdate, $todate) {

        /*$sql = "SELECT dcr_summary.dDCRDate,dcr_summary.cDCRRemarks,requirement_master.cRequirementTitle,
          CMREQ.cClientName as cReqClientName,property_master.cPropertyName,
          CMPROP.cClientName as cPropClientName,dcr_detail.iRequirementId,dcr_detail.cDCRSummary
          FROM dcr_detail
          LEFT JOIN dcr_summary ON dcr_detail.iDCRId=dcr_summary.iDCRId
          LEFT JOIN requirement_master ON dcr_detail.iRequirementId=requirement_master.iRequirementId
          LEFT JOIN property_master ON dcr_detail.iPropertyId=property_master.iPropertyId
          LEFT JOIN client_master as CMREQ ON dcr_detail.iClientReqId=CMREQ.iClientId
          LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId=CMPROP.iClientId
          WHERE dcr_detail.iTaskId='" . $taskid . "' AND dcr_summary.iUserId = '".$userid."' AND dcr_summary.dDCRDate BETWEEN '" . $formdate . "' AND '" . $todate . "' AND dcr_detail.bDelete=0
          ORDER BY dcr_summary.dDCRDate ASC"; */
        ...
        $aColumns = array("dcr_detail.iDCRId,dcr_summary.dDCRDate,dcr_summary.cDCRRemarks,requirement_master.cRequirementTitle,
                    CMREQ.cClientName as cReqClientName,property_master.cPropertyName,
                    CMPROP.cClientName as cPropClientName,dcr_detail.iRequirementId,dcr_detail.cDCRSummary,dcr_detail.iClientReqId");

        // DB table to use
        // DB table to use
        $sIndexColumn = 'dcr_detail.iDCRDetailId';
        $sTable = 'dcr_detail';
        $where = " dcr_detail.iTaskId='".$taskid."' AND dcr_summary.iUserId ='".$userid."' AND dcr_summary.dDCRDate BETWEEN '".$formdate."' AND '".$todate."' AND dcr_detail.bDelete=0";
        ...
        if (isset($iSortCol_0)) {
            for ($i = 0; $i < intval($iSortingCols); $i++) {
                ...
                if ($bSortable == 'true') {
                    $col = intval($this->db->escape_str($iSortCol));
                    $this->db->order_by('dcr_summary.dDCRDate', $this->db->escape_str($sSortDir));
                }
            }
        }
...
//            $sWhere = substr_replace( $sWhere, "", -3 );
        ...
        $this->db->join("dcr_summary", "dcr_detail.iDCRId=dcr_summary.iDCRId", "LEFT");
        $this->db->join("requirement_master", "dcr_detail.iRequirementId=requirement_master.iRequirementId", "LEFT");
        $this->db->join("property_master", "dcr_detail.iPropertyId=property_master.iPropertyId", "LEFT");
        $this->db->join("client_master as CMREQ", "dcr_detail.clientid=CMREQ.iClientId", "LEFT");
        $this->db->join("client_master as CMPROP", "dcr_detail.clientid=CMPROP.iClientId", "LEFT");
        $this->db->where($sWhere, null, false);
        $rResult = $this->db->get();
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $dt = trim($aRow['dDCRDate']);
            $splitdt = explode('-', $dt);
            $Date = $splitdt[2] . "/" . $splitdt[1] . "/" . $splitdt[0];


            if ($aRow['cPropClientName'] == "") {
                $cleintsname = $this->getClientName($aRow['iClientReqId']);
            } else {
                $cleintsname = $aRow['cPropClientName'];
            }

            $row[0] = $sno;
            $row[1] = $Date;
            $row[2] = $cleintsname ? $cleintsname : '';
            $row[3] = $aRow['cRequirementTitle'] ? $aRow['cRequirementTitle'] : '';
            $row[4] = $aRow['cPropertyName'] ? $aRow['cPropertyName'] : '';
    ...
    function get_dashboardData() {
        ...
        $to_day = date('Y-m-d');
        $today_datetime1 = date('Y-m-d 00:00:00');
        $today_datetime2 = date('Y-m-d 23:59:59');

        $sql_rf = "SELECT cRequirementTitle FROM requirement_master 		
                        WHERE 
                        requirement_master.bActive=1 
                        AND requirement_master.bDelete=0                    
                        ORDER BY dDate ASC LIMIT 0,10";

        $query_rf = $this->db->query($sql_rf);
        $result_rf = $query_rf->result_array();


        $sql_psr = "SELECT cPropertyName FROM property_master 		
                        WHERE property_master.bActive=1 
                        AND property_master.bDelete=0                    
                        ORDER BY dDate ASC LIMIT 0,10";

        $query_psr = $this->db->query($sql_psr);
        $result_psr = $query_psr->result_array();


        /*         * ************* */
        $sql_rf_latest = "SELECT cRequirementTitle FROM requirement_master 		
                        WHERE 
                         requirement_master.bActive=1 
                        AND requirement_master.bDelete=0                    
                        ORDER BY dDate DESC LIMIT 0,10";

        $query_rf_latest = $this->db->query($sql_rf_latest);
        $result_rf_latest = $query_rf_latest->result_array();


        $sql_psr_latest = "SELECT cPropertyName FROM property_master 		
                        WHERE property_master.bActive=1 
                        AND property_master.bDelete=0                    
                        ORDER BY dDate DESC LIMIT 0,10";

        $query_psr_latest = $this->db->query($sql_psr_latest);
        $result_psr_latest = $query_psr_latest->result_array();


        $sql_reminder = "SELECT iTaskAssignId,DATE_FORMAT(dTaskTargetDateTime,'%d/%m/%Y') AS dTaskTargetDateTime,cTaskSummary FROM task_assign 		
                        WHERE  dTaskAssignDateTime BETWEEN '" . $today_datetime1 . "' AND '" . $today_datetime2 . "'
                        AND task_assign.bDelete=0                    
                        ORDER BY dTaskAssignDateTime DESC LIMIT 0,50";

        $query_reminder = $this->db->query($sql_reminder);
        $result_reminder = $query_reminder->result_array();


        /* $sql_task = "SELECT task_master.cTaskName,task_master.iTaskId,
          (SELECT COUNT(iTaskId) from task_assign where task_master.iTaskId = task_assign.iTaskId
          AND  task_assign.dTaskAssignDateTime BETWEEN '" . $today_datetime1 . "' AND '" . $today_datetime2 . "') as tasktypecount
          FROM task_master
          ORDER BY task_master.cTaskName ASC";

         */

        $sql_task = "SELECT task_master.cTaskName,task_master.iTaskId,
                    (SELECT COUNT(iTaskId) from dcr_detail left join dcr_summary
                    on dcr_summary.iDCRId = dcr_detail.iDCRId
                    where 
                    task_master.iTaskId = dcr_detail.iTaskId 
                    AND  dcr_summary.dDCRDate = '" . $to_day . "') as tasktypecount 
                    FROM task_master 
                    ORDER BY task_master.cTaskName ASC";

        $query_task = $this->db->query($sql_task);
        $result_task = $query_task->result_array();


        $data_result_array = array();
        $data_result_array['rf'] = $result_rf;
        $data_result_array['psr'] = $result_psr;
        $data_result_array['rf_latest'] = $result_rf_latest;
        $data_result_array['psr_latest'] = $result_psr_latest;
        $data_result_array['reminder'] = $result_reminder;
        $data_result_array['task'] = $result_task;
    ...
    function get_all_website_logo_master() {
        $sql = "SELECT website_logo_master.iWebsiteLogoId,website_logo_master.cWebsiteLogoFileName,website_logo_master.cWebsiteLogoFilePath,CASE website_logo_master.bActive WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bActive' FROM website_logo_master WHERE website_logo_master.bDelete=0";
    ...
    function website_logo_master_get_by_id($id) {
        $sql = "SELECT * FROM website_logo_master WHERE website_logo_master.iWebsiteLogoId='".$id."'";
    ...
    function add_website_logo_master($data) {
        
        $WebsiteLogoFileName = trim($data['hfWebsiteLogoFileName']);
        $WebsiteLogoFilePath = trim($data['hfWebsiteLogoFilePath']);
        $Active = (int) trim($data['chkActive']);
        
        $addData = array(
            'cWebsiteLogoFilePath' => $WebsiteLogoFilePath,
            'cWebsiteLogoFileName' => $WebsiteLogoFileName,
            'bActive' => $Active,
        );

        $insert = $this->db->insert('website_logo_master', $addData);
    ...
    function edit_website_logo_master($data) {

        $hfWebsiteLogoId = trim($data['hfWebsiteLogoId']);
        ...
        if(isset($data['hfWebsiteLogoFileName']) && (!empty($data['hfWebsiteLogoFileName']))){

            if(isset($data['hfWebsiteLogoExistingFilePath']) && (!empty($data['hfWebsiteLogoExistingFilePath']))){
                $WebsiteLogoExistingFilePath = trim($data['hfWebsiteLogoExistingFilePath']);
                if (file_exists($WebsiteLogoExistingFilePath)) {
                    $delete = unlink($WebsiteLogoExistingFilePath);  
                } 
            }

            $editData['cWebsiteLogoFileName'] = trim($data['hfWebsiteLogoFileName']);
            $editData['cWebsiteLogoFilePath'] = trim($data['hfWebsiteLogoFilePath']);
        }

                  $this->db->where('iWebsiteLogoId', $hfWebsiteLogoId);
        $update = $this->db->update('website_logo_master', $editData);
    ...
    function delete_website_logo_master($id) {

        $sql = "UPDATE website_logo_master SET website_logo_master.bActive=0,website_logo_master.bDelete=1 WHERE website_logo_master.iWebsiteLogoId='".$id."'";
    ...
    function requisitionhistory_deliveries($requirementId){
        $sql = 'SELECT (SELECT cClientName FROM client_master WHERE client_master.iClientId = dcr_detail.clientid) AS clientsname, dcr_summary.dDCRDate, user_master.cName,dcr_summary.cDCRRemarks, task_master.cTaskName, requirement_master.cRequirementTitle, CMREQ.cClientName as cReqClientName, property_master.cPropertyName, CMPROP.cClientName as cPropClientName, dcr_detail.iRequirementId, dcr_detail.cDCRSummary FROM dcr_detail LEFT JOIN dcr_summary ON dcr_detail.iDCRId = dcr_summary.iDCRId LEFT JOIN user_master ON dcr_summary.iUserId = user_master.iUserId LEFT JOIN task_master ON dcr_detail.iTaskId = task_master.iTaskId LEFT JOIN requirement_master ON dcr_detail.iRequirementId = requirement_master.iRequirementId LEFT JOIN property_master ON dcr_detail.iPropertyId = property_master.iPropertyId LEFT JOIN client_master as CMREQ ON dcr_detail.iClientReqId = CMREQ.iClientId LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId = CMPROP.iClientId WHERE dcr_detail.iRequirementId = '. $requirementId .' AND task_master.cTaskName = "Delivery" AND dcr_detail.bDelete = 0 ORDER BY dcr_summary.dDCRDate ASC';
    ...
    function requisitionhistory_inspections($requirementId){
        $sql = 'SELECT (SELECT cClientName FROM client_master WHERE client_master.iClientId = dcr_detail.clientid) AS clientsname, dcr_summary.dDCRDate, user_master.cName,dcr_summary.cDCRRemarks, task_master.cTaskName, requirement_master.cRequirementTitle, CMREQ.cClientName as cReqClientName, property_master.cPropertyName, CMPROP.cClientName as cPropClientName, dcr_detail.iRequirementId, dcr_detail.cDCRSummary FROM dcr_detail LEFT JOIN dcr_summary ON dcr_detail.iDCRId = dcr_summary.iDCRId LEFT JOIN user_master ON dcr_summary.iUserId = user_master.iUserId LEFT JOIN task_master ON dcr_detail.iTaskId = task_master.iTaskId LEFT JOIN requirement_master ON dcr_detail.iRequirementId = requirement_master.iRequirementId LEFT JOIN property_master ON dcr_detail.iPropertyId = property_master.iPropertyId LEFT JOIN client_master as CMREQ ON dcr_detail.iClientReqId = CMREQ.iClientId LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId = CMPROP.iClientId WHERE dcr_detail.iRequirementId = '. $requirementId .' AND task_master.cTaskName = "Client Inspection" AND dcr_detail.bDelete = 0 ORDER BY dcr_summary.dDCRDate ASC';
