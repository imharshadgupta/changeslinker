...
class Dashboard_model extends CI_Model {
  ...
  /*function user_pending_task_list_all($UserId)
	...
	{       
			...
			$mail->IsHTML(true);
			$mail->SetFrom('support@linkersindia.com', 'Linkers');     //Who is sending the email
			$mail->AddReplyTo($RecipientEmailAddress, 'Linkers');  //Email address that receives the response
...
/*		if($DCRMode=='Replace')
	...
	{
		$sql="SELECT property_master.iPropertyId,property_master.cPropertyName,current_status_master.cCurrentStatusName,CASE property_master.bAcceptTermsAndConditions WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bAcceptTermsAndConditions',CASE property_master.bActive WHEN 1 THEN 'Yes' ELSE 'No' END AS 'bActive' FROM property_master LEFT JOIN current_status_master ON property_master.iCurrentStatusId=current_status_master.iCurrentStatusId WHERE property_master.bDelete=0";
		if( $client_id != 0 ){
...
//		$pdf->SetHeaderData(PDF_HEADER_LOGO, $PDF_HEADER_LOGO_WIDTH, $PDF_HEADER_TITLE, $PDF_HEADER_STRING);
		...
		$pdf->Cell(0, 5, "Linkers India - Property Proposal", $border=0, $ln=1, $align='C', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M');
...
//				$tbl.="</tr>";
				...
				{	  
					$pdf->SetFont('helvetica', 'B', 11);  
						$pdf->Cell(20, 5, "Images :", $border=0, $ln=1, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M');
					$pdf->SetFont('helvetica', '', 11);  
							$SNo=1;
							foreach($AttachmentRows as $docrow)
							{
								...
								$pdf->Image($ImgUrl, 100, $pdf->gety(), 80, 60, $type='', $link='', $align='', $resize=false, $dpi=200, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) ;
						$pdf->Cell(20, 5, "", $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M');
						$pdf->Cell(20, 60, $SNo . ") ". $AttachmentTitle, $border=0, $ln=1, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M');
