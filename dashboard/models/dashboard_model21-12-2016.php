...
//       // error_reporting(E_ALL);
    ...
    function get_drill_down_datatable($taskid, $userid, $formdate, $todate) {
        ...
        $aColumns = array("dcr_summary.dDCRDate,dcr_summary.cDCRRemarks,requirement_master.cRequirementTitle,
                    CMREQ.cClientName as cReqClientName,property_master.cPropertyName,
                    CMPROP.cClientName as cPropClientName,dcr_detail.iRequirementId,dcr_detail.cDCRSummary");
...
//            $sWhere = substr_replace( $sWhere, "", -3 );
        ...
        $this->db->join("client_master as CMREQ", "dcr_detail.iClientReqId=CMREQ.iClientId", "LEFT");
        $this->db->join("client_master as CMPROP", "dcr_detail.iClientPropId=CMPROP.iClientId", "LEFT");
        $this->db->where($sWhere, null, false);
        $rResult = $this->db->get();
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $row[0] = $sno;
            $row[1] = $Date;
            $row[2] = $aRow['cPropClientName'] ? $aRow['cPropClientName'] : '';
