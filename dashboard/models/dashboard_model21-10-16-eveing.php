...
//        AND property_master.bDelete=0)';
    ...
    function search_by_text_in_client($txtSearch, $cmbPropertyCategory, $cmbPropertyType, $cmbPropertyStatus, $cmbState, $cmbCity, $cmbLocation) {
        ...
        if ($cmbPropertyStatus != "") {
            $sql .= " AND datewise_dcr_print_reportproperty_master.iPropertyStatusId = '" . $cmbPropertyStatus . "'";
        }
            ...
            foreach ($data['cmbTask'] as $key => $val) {
                if (!empty($data['cmbTask'][$key]) && ($data['cmbTask'][$key] != '')) {
                    $addDCRDetail = array(
                        'iDCRId' => $DCRId,
                        'iTaskId' => $data['cmbTask'][$key],
                        'iClientReqId' => $reqclientid ? $reqclientid : '',
                        'iRequirementId' => $data['cmbRequirement'][$key],
...
//            $sWhere = substr_replace( $sWhere, "", -3 );
    ...
    function get_dashboardData() {
        ...
        $sql_reminder = "SELECT DATE_FORMAT(dTaskTargetDateTime,'%d/%m/%Y') AS dTaskTargetDateTime,cTaskSummary FROM task_assign 		
