...
class Cron_model extends CI_Model {
	...
	function daily_tracker_report_mailer($CurrentLocalDate)
	{
		...
		$tbl.="<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">";
		$tbl.="<tr>";
		$tbl.="<td align=\"center\" style=\"font-size:15px;text-decoration:underline;\"><b>7. Task Summary</b></td>";
		$tbl.="</tr>";
		$tbl.="<tr>";
		$tbl.="<td>&nbsp;</td>";
		$tbl.="</tr>";
		...
		if($query77)
		{
			if($query77->num_rows() > 0)
			{
				foreach($query77->result_array() as $row77)
				{
					...
					$tbl.="<tr>";
					$tbl.="<td align=\"center\"><b>".$StaffName."</b></td>";	
					$tbl.="</tr>";
					$tbl.="<tr>";
					$tbl.="<td align=\"center\">";
					$tbl.="<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\">
					...
					$sql7 = "SELECT task_master.cTaskName,CMREQ.cClientName as cReqClientName,requirement_master.cRequirementTitle,CMPROP.cClientName as cPropClientName,property_master.cPropertyName,dcr_detail.cDCRSummary
							 ...
							 LEFT JOIN client_master as CMREQ ON dcr_detail.iClientReqId=CMREQ.iClientId LEFT JOIN client_master as CMPROP ON dcr_detail.iClientPropId=CMPROP.iClientId WHERE dcr_detail.iDCRId='".$DCRId."'";
